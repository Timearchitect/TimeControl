...
abstract class Ability implements Cloneable {
  ...
  void assambleTooltip(String ..._toolTip) {
...
class ThrowDagger extends Ability {//---------------------------------------------------    ThrowDagger   ---------------------------------
  ...
  @Override
    ...
    if (abs(owner.vx)>threashold  ||  abs(owner.vy)>threashold) {
      if (abs(owner.keyAngle-owner.angle)<5) {
        ...
        if (alternate) {
          projectiles.add( new ArchingIceDagger(owner, int( owner.cx), int(owner.cy), 30, owner.playerColor, 1000, owner.keyAngle, (owner.keyAngle-owner.angle)*6, owner.ax*24, owner.ay*24, int(damage+damageMod*.3)));
      ...
      //}
      projectiles.add( new ArchingIceDagger(owner, int( owner.cx+cos(radians(owner.angle))*-55), int(owner.cy+sin(radians(owner.angle))*-55), 30, owner.playerColor, 1000, owner.angle, owner.angle, sin(radians(owner.angle+140))*24, -cos(radians(owner.angle+140))*24, int(damage+damageMod*.3)*2).addBuff(new Cold(owner, 3000)));
      projectiles.add( new ArchingIceDagger(owner, int( owner.cx+cos(radians(owner.angle))*-55), int(owner.cy+sin(radians(owner.angle))*-55), 30, owner.playerColor, 1000, owner.angle, owner.angle+180, sin(radians(owner.angle+40))*24, -cos(radians(owner.angle+40))*24, int(damage+damageMod*.3)*2).addBuff(new Cold(owner, 3000)));

      projectiles.add( new ArchingIceDagger(owner, int( owner.cx+cos(radians(owner.angle))*-125), int(owner.cy+sin(radians(owner.angle))*-125), 30, owner.playerColor, 1000, owner.angle, owner.angle, sin(radians(owner.angle+140))*20, -cos(radians(owner.angle+140))*20, int(damage+damageMod*.3)*2).addBuff(new Cold(owner, 3000)));
      projectiles.add( new ArchingIceDagger(owner, int( owner.cx+cos(radians(owner.angle))*-125), int(owner.cy+sin(radians(owner.angle))*-125), 30, owner.playerColor, 1000, owner.angle, owner.angle+180, sin(radians(owner.angle+40))*20, -cos(radians(owner.angle+40))*20, int(damage+damageMod*.3)*2).addBuff(new Cold(owner, 3000)));
      projectiles.add( new Slash(owner, int( owner.cx), int(owner.cy), 32, owner.playerColor, int(slashDuration), owner.angle+90, 22, slashRange, sin(owner.keyAngle)*5, cos(owner.keyAngle)*5, slashdamage, true));
      projectiles.add( new Slash(owner, int( owner.cx), int(owner.cy), 32, owner.playerColor, int(slashDuration), owner.angle-90, -22, slashRange, sin(owner.keyAngle)*5, cos(owner.keyAngle)*5, slashdamage, true));
      particles.add( new Feather(100, int(owner.cx+cos(radians(owner.keyAngle-45))*75), int(owner.cy+sin(radians(owner.keyAngle-45))*75), owner.ax*10, owner.ay*10, -10, owner.playerColor));
      particles.add( new Feather(100, int(owner.cx+cos(radians(owner.keyAngle+45))*75), int(owner.cy+sin(radians(owner.keyAngle+45))*75), owner.ax*10, owner.ay*10, 40, owner.playerColor));
    }
  ...
  void reset() {
    super.reset();
    cooldownTimer = int(cooldownTimer-attackSpeedMod*1.8);
  }
}
...
class Pistol extends Ability implements AmmoBased {//---------------------------------------------------    Pistol   ---------------------------------
  ...
  @Override
    ...
    owner.addBuff(new SpeedBuff(owner, 1000, 0.12).apply(BuffType.ONCE));
    owner.addBuff(new DamageBuff(owner, 2000, int(damage*0.5)).apply(BuffType.ONCE));
...
class Revolver extends Ability implements AmmoBased {//---------------------------------------------------    Revolver   ---------------------------------
  ...
  void reset() {
    ...
    ammo=0;
    cooldown=0;
    cooldownTimer=int(250-attackSpeedMod*2.6); //100
  }
}
  ...
  @Override
    ...
    if ((!reverse || owner.reverseImmunity)&&(!freeze || owner.freezeImmunity) &&  active && !owner.dead) {
      ...
      if (MAX_FORCE>forceAmount) { 
        ...
        if (!owner.stealth)particles.add( new  Particle(int(owner.cx+cos(radians(owner.angle))*75), int(owner.cy+sin(radians(owner.angle))*75), 0, 0, int(MAX_FORCE*1.5), 30, color(255, 0, 255)));
      }
    }
  ...
  @Override
    ...
    if ((!reverse || owner.reverseImmunity ) ) {
      if (!owner.dead && (!freeze || owner.freezeImmunity)&& active && channeling) {
        //stamps.add( new AbilityStamp(owner.index, int(owner.x), int(owner.y), energy, active, channeling, cooling, regen, hold));
        stamps.add( new AbilityStamp(this));
        particles.add(new ShockWave(int(owner.cx+cos(radians(owner.angle))*75), int(owner.cy+sin(radians(owner.angle))*75), int(forceAmount), 32, int(forceAmount*2), owner.playerColor));
...
class Stealth extends Ability {//---------------------------------------------------    Stealth Ability  ---------------------------------
  ...
  @Override
    ...
    //}
    particles.add(new   AfterImage(int(owner.cx), int( owner.cy), 0, 0, owner.angle, 0, 0, 300, 1000, owner.playerColor, owner)); 
...
class Combo extends Ability {//---------------------------------------------------    Combo   ---------------------------------
  ...
  void attack() {
    ...
    case 4:
      shakeTimer+=10;
      particles.add(new ShockWave(int(owner.cx), int(owner.cy), 20, 16, 175, owner.playerColor));
      projectiles.add(new Stab( owner, int(owner.cx+cos(radians(owner.angle))*-350), int(owner.cy+sin(radians(owner.angle))*-350), 160, owner.playerColor, 180, owner.angle+170, 2, 300, 35, cos(radians(owner.angle+180))*0, sin(radians(owner.angle+170))*0, int((damage+damageMod*0.3)*1.3), false));
  ...
  @Override
    ...
    if (step==3) {
      ...
      if (!owner.stealth)particles.add(new Particle(int(owner.cx), int(owner.cy), owner.vx, owner.vy, 120, 50, WHITE));
    } else {
      owner.MAX_ACCEL= MODIFIED_MAX_ACCEL;
...
class GranadeLauncher extends Ability {//---------------------------------------------------    TimeBomb   ---------------------------------
  ...
  @Override
    ...
    for (int i=0; i<6; i++) {
      ...
      float sprayVelocity=random(10*0.5);
      particles.add(new Spark( 1000, int( owner.cx), int(owner.cy), cos(radians(sprayAngle))*sprayVelocity, sin(radians(sprayAngle))*sprayVelocity, 6, sprayAngle, owner.playerColor));
    }
    //if (int(random(5))!=0) {
...
class Bazooka extends Ability {//---------------------------------------------------    Bazooka   ---------------------------------
  ...
  @Override
    ...
    case 3:
      ...
      Missile m= new Missile(owner, int( owner.cx), int(owner.cy), 40, owner.playerColor, 1100+int(random(400)), owner.angle, cos(radians(owner.angle))*60, sin(radians(owner.angle))*60, damage, true);
      m.blastRadius=400;
...
class Stars extends Ability {//---------------------------------------------------    Stars   ---------------------------------
  ...
  @Override
    ...
    deChannel();
    regen=true;
    cooldownTimer=int(1000-attackSpeedMod*10);
  }
}

class RapidFire extends Ability {//---------------------------------------------------    RapidFire   ---------------------------------
  float accuracy = 1, MODIFIED_ANGLE_FACTOR=-0.0008, r=50, maxR=100, damage;  
  ...
  void reset() {
    super.reset();
    PastTime=stampTime;
    Interval = int(DEFAULT_INTERVAL-attackSpeedMod);
    // channeling=false;
    // active=false;
    // hold=false;
    // deactivate();
...
class CrossFire extends RapidFire {//---------------------------------------------------    CrossFire   ---------------------------------
  float accuracy = 1, MODIFIED_ANGLE_FACTOR=-0.0008, r=50, maxR=100, damage, crossAngle;  
  int Interval=140;
  long  PastTime;
  //int projectileDamage=5;
  ...
  @Override
    ...
    if (!owner.stealth) {
      ...
      strokeWeight(12);
      if (r<maxR)r*=1.1;
      line(int( owner.cx+cos(radians(owner.angle+90))*owner.w), int( owner.cy+sin(radians(owner.angle+90))*owner.w), int( owner.cx+cos(radians(owner.angle+90))*owner.w*r*.005), int( owner.cy+sin(radians(owner.angle+90))*owner.w*r*.005));
      line(int( owner.cx+cos(radians(owner.angle-90))*owner.w), int( owner.cy+sin(radians(owner.angle-90))*owner.w), int( owner.cx+cos(radians(owner.angle-90))*owner.w*r*.005), int( owner.cy+sin(radians(owner.angle-90))*owner.w*r*.005));
    }
  }
  void reset() {
    super.reset();
    PastTime=stampTime;
    Interval=int(140-attackSpeedMod*1.4); //100
    ...
    if ((!reverse || owner.reverseImmunity ) ) {
      if (!owner.dead && (!freeze || owner.freezeImmunity)&& active && channeling) {
        ...
        for (int i=0; sutainCount/10>i; i++) {
          ...
          projectiles.add( new Needle(owner, int( owner.cx+cos(radians(owner.angle))*owner.w), int(owner.cy+sin(radians(owner.angle))*owner.w), 60, owner.playerColor, 700, owner.angle+InAccurateAngle, cos(radians(owner.angle+InAccurateAngle))*projectileSpeed, sin(radians(owner.angle+InAccurateAngle))*projectileSpeed, int(damage)));
        }
  ...
  void passive() {
    ...
    if (!owner.stealth) {   
      ...
      if (cooldown<stampTime && active) {
        ...
        //   line(owner.cx+xOffset, owner.cy+yOffset, int( owner.cx+cos(radians(-inAccurateAngle*3.25+owner.angle))*2000), int(owner.cy+sin(radians(-inAccurateAngle*3.25+owner.angle))*2000));
        aimLine(nullRange, 3000, inAccurateAngle*3.25);
        aimLine(nullRange, 3000, -inAccurateAngle*3.25);
        noFill();
        ...
        aimLine(nullRange, 3000, inAccurateAngle*0.25);
        aimLine(nullRange, 3000, -inAccurateAngle*0.25);
        ...
        aimLine(nullRange, 3000, inAccurateAngle*0.5);
        aimLine(nullRange, 3000, -inAccurateAngle*0.5);
        strokeWeight(3);
  ...
  void release() {
    if ((!reverse || owner.reverseImmunity ) ) {
      ...
      if (!owner.dead && (!freeze || owner.freezeImmunity)&& active && channeling) {
        ...
        Spike arrow= new Spike(owner, int( owner.cx+cos(radians(owner.angle))*nullRange), int(owner.cy+sin(radians(owner.angle))*nullRange), 55, owner.playerColor, 800, owner.angle+sin(radians(count))*inAccurateAngle, cos(radians(owner.angle+sin(radians(count))*inAccurateAngle))*12*sutainCount, sin(radians(owner.angle+sin(radians(count))*inAccurateAngle))*12*sutainCount, int((damage-inAccurateAngle+damageMod*.2)*sutainCount*.1));
        arrow.size=90;
...
class Battery extends Ability {//---------------------------------------------------    Battery  Ability  ---------------------------------
  ...
  Battery() {
    ...
    unlockCost=2000;
    assambleTooltip("Tap");
    maxInterval=int(DEFAULT_MAX_INTERVAL-attackSpeedMod*0.04);
  } 
...
class AssaultBattery extends Ability {//---------------------------------------------------    Battery   ---------------------------------
  ...
  @Override
    ...
    stroke(owner.playerColor);
    if (flip==1) {
      arc(owner.cx, owner.cy, 2000, 2000, radians(owner.angle-accuracy*1.0), radians(owner.angle));
    } else {
      arc(owner.cx, owner.cy, 2000, 2000, radians(owner.angle), radians(owner.angle+accuracy*1.0));
    }
    strokeWeight(1);
    flip*=-1;
...
class MissileLauncher extends Ability {//---------------------------------------------------    MissileLauncher   ---------------------------------
  ...
  @Override
    ...
    case 0:
      owner.ANGLE_FACTOR= MODIFIED_ANGLE_FACTOR;
      projectiles.add( new Missile(owner, int( owner.cx+cos(radians(owner.angle+90))*50), int(owner.cy+sin(radians(owner.angle+90))*50), 30, owner.playerColor, duration, owner.angle, cos(radians(owner.angle))*shootSpeed, sin(radians(owner.angle))*shootSpeed, int(damage+damageMod*.2), false).addBuff(new CriticalHit(owner, critChance+criticalChanceMod, (critDamage+criticalDamageMod))));
      particles.add( new  Particle(int(owner.cx+cos(radians(owner.angle+90))*50), int(owner.cy+sin(radians(owner.angle+90))*50), 0, 0, 50, 50, WHITE));
    ...
    case 3:
      ...
      break;
    case 4:
      projectiles.add( new Missile(owner, int( owner.cx+cos(radians(owner.angle+100))*50), int(owner.cy+sin(radians(owner.angle+100))*50), 30, owner.playerColor, duration, owner.angle+20, cos(radians(owner.angle+20))*shootSpeed, sin(radians(owner.angle+20))*shootSpeed, int(damage+damageMod*.2), false).addBuff(new CriticalHit(owner, critChance+criticalChanceMod, (critDamage+criticalDamageMod))));
      particles.add( new  Particle(int(owner.cx+cos(radians(owner.angle-90))*50), int(owner.cy+sin(radians(owner.angle-90))*50), 0, 0, 50, 50, WHITE));
...
class AutoGun extends Ability {//---------------------------------------------------    AutoGun   ---------------------------------
  ...
  void calcAngle(Player target) {
    ...
    strokeWeight(1);
    stroke(owner.playerColor);
    line(target.cx, target.cy, int(owner.cx+cos(radians(owner.angle))*85), int(owner.cy+sin(radians(owner.angle))*85));
...
class SeekGun extends Ability {//---------------------------------------------------    SeekGun Abilty  ---------------------------------
    ...
    if ((!reverse || owner.reverseImmunity ) ) {
      if (!owner.dead && (!freeze || owner.freezeImmunity)&& active && channeling) {
        ...
        for (Player t : targets) {
          ...
          HomingMissile p=new HomingMissile(owner, int( owner.cx+cos(radians(tempAngle))*50), int(owner.cy+sin(radians(tempAngle))*50), 60+20*targets.size(), owner.playerColor, 1400, owner.angle, cos(radians(owner.angle))*30, sin(radians(owner.angle))*30, int(damage+damageMod*.7+(15+damageMod*.5)*targets.size()));
          p.angle=tempAngle;
          p.locking();  
  ...
  float  calcAngleBetween(float x, float y, float x2, float y2) {
...
class CutThroat extends SeekGun { 
  ...
  void press() {
    if (!owner.dead) {
      ...
      particles.add( new Star(1500, int(owner.cx+cos(radians(owner.angle))*-75), int(owner.cy+sin(radians(owner.angle))*-75), 0, 0, 400, 0.7, owner.playerColor) );
    }
    super.press();
  }
    ...
    if ((!reverse || owner.reverseImmunity ) ) {
      if (!owner.dead && (!freeze || owner.freezeImmunity)&& active && channeling) {
        ...
        if (targets.size()>0) {
          minAngle=70;
          expandRate=0.06;
          particles.add(new RShockWave(int(owner.cx), int(owner.cy), 400, 30, 850, WHITE));
          particles.add( new Star(1800, int(owner.cx+cos(radians(owner.angle))*75), int(owner.cy+sin(radians(owner.angle))*75), 0, 0, 200+100*targets.size(), 0.7, owner.playerColor) );
        } else {
          minAngle=30;
          expandRate+=0.02;
  ...
  @Override
    ...
    if (slaughter && !freeze) {
      ...
      if (targetTurnIndex<targets.size()) {   
        ...
        projectiles.add( new Slash(owner, int( owner.cx+cos(radians(owner.angle))*65), int(owner.cy+sin(radians(owner.angle))*65), 32, owner.playerColor, 130, owner.angle+100, 24, 140, sin(owner.keyAngle)*5, cos(owner.keyAngle)*5, int((damage+damageMod*0.1)*(2.5-targets.size()*.07)), false).hitOnce(true).addBuff(new CriticalHit(owner, critChance+criticalChanceMod, (critDamage+criticalDamageMod))));
  ...
  @Override
    ...
    if ((!reverse || owner.reverseImmunity ) ) {
      if (!owner.dead && (!freeze || owner.freezeImmunity)&& active && channeling) {
        ...
        //stamps.add( new AbilityStamp(owner.index, int(owner.x), int(owner.y), energy, active, channeling, cooling, regen, hold));
        stamps.add( new AbilityStamp(this));
        projectiles.add( new Slash(owner, int( owner.cx), int(owner.cy), 40, owner.playerColor, 120, owner.angle+100, 24, 80, sin(owner.keyAngle)*5, cos(owner.keyAngle)*5, slashdamage, false));
        regen=true;
        action();
    ...
    if (energy>=maxEnergy-15) {
      ...
      for (Player p : players) {
        if (!p.dead && p.targetable) {
          ...
          HomingMissile h= new HomingMissile(owner, int( owner.cx), int(owner.cy), 70, owner.playerColor, 5000, owner.angle, cos(radians(owner.angle+customAngle))*30, sin(radians(owner.angle+customAngle))*initialSpeed, int(damage+damageMod*.4));
          h.target=p;
          projectiles.add(h.addBuff(new CriticalHit(owner, critChance+criticalChanceMod, (critDamage+criticalDamageMod))));
          customAngle=90;

          h= new HomingMissile(owner, int( owner.cx), int(owner.cy), 70, owner.playerColor, 5000, owner.angle, cos(radians(owner.angle+customAngle))*30, sin(radians(owner.angle+customAngle))*initialSpeed, int(damage+damageMod*.4));
          h.target=p;
          projectiles.add(h.addBuff(new CriticalHit(owner, critChance+criticalChanceMod, (critDamage+criticalDamageMod))));
          customAngle=0;
        }
...
class DeployShield extends Ability {//---------------------------------------------------    DeployShield   ---------------------------------
  ...
  @Override
    ...
    if (owner.health>=owner.maxHealth*.5) {
      ...
      projectiles.add( mergePayload(new Shield( owner, int( owner.cx+cos(radians(owner.angle-25))*220), int(owner.cy+sin(radians(owner.angle-25))*220), owner.playerColor, 11000, owner.angle+90, damage ), new Containable[]{   
        new IceDagger(owner, int( owner.cx), int(owner.cy), 25, owner.playerColor, 300, owner.angle-15, cos(radians(owner.angle-15))*50, sin(radians(owner.angle-15))*50, int(damage+damageMod*.5)), 
        new IceDagger(owner, int( owner.cx), int(owner.cy), 25, owner.playerColor, 300, owner.angle, cos(radians(owner.angle))*50, sin(radians(owner.angle))*50, int(damage+damageMod*.5)), 
        new IceDagger(owner, int( owner.cx), int(owner.cy), 25, owner.playerColor, 300, owner.angle+15, cos(radians(owner.angle+15))*50, sin(radians(owner.angle+15))*50, int(damage+damageMod*.5))
        }));
      projectiles.add( mergePayload(new Shield( owner, int( owner.cx+cos(radians(owner.angle))*200), int(owner.cy+sin(radians(owner.angle))*200), owner.playerColor, 11000, owner.angle+90, damage ), new Containable[]{   
        new IceDagger(owner, int( owner.cx), int(owner.cy), 25, owner.playerColor, 300, owner.angle-15, cos(radians(owner.angle-15))*50, sin(radians(owner.angle-15))*50, int(damage+damageMod*.5)), 
        new IceDagger(owner, int( owner.cx), int(owner.cy), 25, owner.playerColor, 300, owner.angle, cos(radians(owner.angle))*50, sin(radians(owner.angle))*50, int(damage+damageMod*.5)), 
        new IceDagger(owner, int( owner.cx), int(owner.cy), 25, owner.playerColor, 300, owner.angle+15, cos(radians(owner.angle+15))*50, sin(radians(owner.angle+15))*50, int(damage+damageMod*.5))
        }));
...
class Explosion extends Ability {//---------------------------------------------------    Explosion   ---------------------------------
  ...
  @Override
    ...
    Container waterRocket= new Rocket(owner, int( owner.cx-cos(radians(owner.angle))*300), int(owner.cy-sin(radians(owner.angle))*300), 50, owner.playerColor, 80, owner.angle, cos(radians(owner.angle))*shootSpeed+owner.vx, sin(radians(owner.angle))*shootSpeed+owner.vy, int(damage*0.3), false);
    payload=new Containable[2];
    payload[0]= new ChargeLaser(owner, 0, 0, 650, owner.playerColor, 150, owner.angle-180, 16, damage*.3 ).parent(waterRocket); 
...
class RandoGun extends Ability {//---------------------------------------------------    RandoGun   ---------------------------------
  ...
  @Override
    ...
    case 14:
      ...
      break;
    case 15:
      projectiles.add( new Missile(owner, int( owner.cx+cos(radians(owner.angle+90))*50), int(owner.cy+sin(radians(owner.angle+90))*50), 30, owner.playerColor, 5000, owner.angle, cos(radians(owner.angle-90))*shootSpeed, sin(radians(owner.angle-90))*shootSpeed, damage, false));
      projectiles.add( new Missile(owner, int( owner.cx+cos(radians(owner.angle+90))*50), int(owner.cy+sin(radians(owner.angle+90))*50), 30, owner.playerColor, 5000, owner.angle, cos(radians(owner.angle+90))*shootSpeed, sin(radians(owner.angle+90))*shootSpeed, damage, false));
      break;
    case 16:
...
class FlameThrower extends Ability {//---------------------------------------------------    FlameThrower   ---------------------------------
  ...
  int alt, count;
  float sutainCount, MAX_sutainCount=50, accuracy, MODIFIED_ANGLE_FACTOR=0.7, MODIFIED_MAX_ACCEL=0.1, damage;
  FlameThrower() {
    super();
    icon=icons[55];
  ...
  void press() {
    ...
    if (!active) {
      active=true;
      projectiles.add( new  Blast(owner, int( owner.cx), int(owner.cy), 0, 150, owner.playerColor, 200, owner.angle, damage));
    }
  }
  void action() {
    if ((!reverse || owner.reverseImmunity) &&  active && !owner.dead ) {
      ...
      if (sutainCount%2<0.5) {
        projectiles.add( new  Blast(owner, int( owner.cx), int(owner.cy), 1+sutainCount, 45, owner.playerColor, 170, owner.angle+InAccurateAngle, int(damage+sutainCount*.01)).addBuff(new Burn( owner, 2000, 0.00005, int(random(500, 2000)))));
...
class SneakBall extends Ability {//---------------------------------------------------    SneakBall  ---------------------------------
  ...
  @Override
    ...
    for (int i=0; i<10; i++) {
      payload[i]= new HomingMissile(owner, int( cos(radians(i*36))*100), int(sin(radians(i*36))*100), 65, owner.playerColor, 4000, owner.angle, cos(radians(owner.angle))*shootSpeed*.1, sin(radians(owner.angle))*shootSpeed*.1, int(damage+damageMod*.4)).parent(ball); 
    ...
    payload[10] =new Bomb(owner, 0, 0, 40, owner.playerColor, 1, owner.angle, 0, 0, int(damage+damageMod*.2), false).parent(ball);
...
class TripleShot extends Ability {//---------------------------------------------------    TripleShot  ---------------------------------
  ...
  @Override
    ...
    owner.pushForce(-18, owner.angle);
    timer=millis();
    projectiles.add( new Needle(owner, int( owner.cx+cos(radians(owner.angle+spreadAngle))*owner.w), int(owner.cy+sin(radians(owner.angle+spreadAngle))*owner.w), 60, owner.playerColor, 1000, owner.angle+spreadAngle, cos(radians(owner.angle+spreadAngle))*36, sin(radians(owner.angle+spreadAngle))*36, int((damage+damageMod*.3)*0.8)).addBuff(new CriticalHit(owner, critChance+criticalChanceMod, (critDamage+criticalDamageMod))));
    projectiles.add( new Needle(owner, int( owner.cx+cos(radians(owner.angle-spreadAngle))*owner.w), int(owner.cy+sin(radians(owner.angle-spreadAngle))*owner.w), 60, owner.playerColor, 1000, owner.angle-spreadAngle, cos(radians(owner.angle-spreadAngle))*36, sin(radians(owner.angle-spreadAngle))*36, int((damage+damageMod*.3)*0.8)).addBuff(new CriticalHit(owner, critChance+criticalChanceMod, (critDamage+criticalDamageMod))));
...
class PoisonDart extends Ability {//---------------------------------------------------    PoisonDart   ---------------------------------
  ...
  @Override
    ...
    projectiles.add( new Needle(owner, int( owner.cx+cos(radians(owner.angle))*owner.w), int(owner.cy+sin(radians(owner.angle))*owner.w), 60, owner.playerColor, 800, owner.angle, cos(radians(owner.angle))*66, sin(radians(owner.angle))*66, int(damage+damageMod*.1)).addBuff(new Poison(owner, 20000+int(200*damageMod), 0.005+(damageMod*.0005))).addBuff(new CriticalHit(owner, critChance+criticalChanceMod, (critDamage+criticalDamageMod))));
    owner.pushForce(-5, owner.angle);
    owner.angle+=random(-angleRecoil, angleRecoil);
    owner.keyAngle=owner.angle;
  }
...
class Ravine extends Ability {//---------------------------------------------------    Shotgun   ---------------------------------
  ...
  void passive() {
    if (charging && !shake && startTime+delay*.5<stampTime) {
      particles.add(new RShockWave(int(owner.cx+cos(radians(owner.angle))*120), int(owner.cy+sin(radians(owner.angle))*120), 500, 30, 150, WHITE));
      shake=true;
      shakeTimer+=15;
    }
    if (charging && startTime+delay<stampTime) {
      ...
      //particles.add(new Flash(50, 6, WHITE)); 
      projectiles.add( new Bomb(owner, int( owner.cx), int(owner.cy), 40, owner.playerColor, 1, owner.angle, cos(radians(owner.angle))*20, sin(radians(owner.angle))*20, int(damage*0.2), false));
      projectiles.add( new  Blast(owner, int( owner.cx), int(owner.cy), 80, 100, owner.playerColor, 200, owner.angle, damage));
      //projectiles.add( new  Blast(owner, int( owner.cx), int(owner.cy), 30, 200, owner.playerColor, 200, owner.angle, damage));
      for (int i=0; i<4; i++) {
        particles.add(  new  Particle(int(owner.cx), int(owner.cy), cos(radians(owner.angle+random(-10, 10)))*15+owner.vx, sin(radians(owner.angle+random(-10, 10)))*15+owner.vy, int(random(40)+10), 500, BLACK));
      }
      for (int i=2; i<9; i++) 
        players.add(new Block(-2, AI, int(owner.cx+cos(radians(owner.angle))*i*100)-40, int(owner.cy+sin(radians(owner.angle))*i*100)-40, 80, 80, 90));
      shakeTimer+=10;
      owner.halt();
      charging=false;
      shake=false;
...
class Artilery extends Ability {//---------------------------------------------------    Artilery Ability  ---------------------------------
  ...
  float r=10, r2=10, maxR=150, MODIFIED_ANGLE_FACTOR = 0.008, spreadAngle=15, retractFactor=0.95, extendFactor=1.11;   
  long timer, tTimer;
  ...
  Artilery() {
    ...
    unlockCost=3600;
    assambleTooltip("Toogle");
    maxR=int(DEFAULT_MAX_INTERVAL-attackSpeedMod*0.150);
    duration=int(200-attackSpeedMod*0.200);
    extendFactor=1.11+attackSpeedMod*0.01;
  } 
...
class LaserSword extends Ability {//---------------------------------------------------    LaserSword Ability ---------------------------------
  ...
  @Override
    ...
    super.activate();
    transformed=!transformed;
    if (l!=null) l.laserLength=int(swordLength*.4);
...
class StunGun extends Ability {//---------------------------------------------------    StunGun   ---------------------------------
  ...
  @Override
    ...
    for (int i=0; i<6; i++) {
      // particles.add(new Particle(int(x), int(y), vx, vy, int(random(30)+10), 800, projectileColor));
      float sprayAngle=random(-40, 40)+owner.angle;
      float sprayVelocity=random(20*0.5);
      particles.add(new Spark( 1000, int( owner.cx), int(owner.cy), cos(radians(sprayAngle))*sprayVelocity, sin(radians(sprayAngle))*sprayVelocity, 6, sprayAngle, owner.playerColor));
    }
    stamps.add( new AbilityStamp(this));
    //particles.add(new LineWave(int(owner.cx+cos(radians(owner.angle))*owner.w*2), int(owner.cy+sin(radians(owner.angle))*owner.w*2), 20, 300, owner.playerColor, owner.angle));
    ...
    Missile m= new Missile(owner, int( owner.cx), int(owner.cy), projectileSize, owner.playerColor, 1100+int(random(400)), owner.angle, cos(radians(owner.angle))*80, sin(radians(owner.angle))*80, int(damage+damageMod*.6), true);
...
class Chivalry extends Ability {
  float damage=5, forceAmount=1, MODIFIED_MAX_ACCEL=0.034, MODIFIED_ANGLE_FACTOR=0.03, damageFactor=1;
  ...
  @Override
    ...
    projectiles.add(new Stab( owner, int(owner.cx-cos(radians(owner.angle))*10), int(owner.cy-sin(radians(owner.angle))*10), 180, owner.playerColor, 200, owner.angle+170, 1, 300, 20, cos(radians(owner.angle+180))*forceAmount*0, sin(radians(owner.angle+170))*forceAmount*0, int(damage*damageFactor+damageMod*0.3), false));
    projectiles.add(new Stab( owner, int(owner.cx-cos(radians(owner.angle))*10), int(owner.cy-sin(radians(owner.angle))*10), 180, owner.playerColor, 200, owner.angle+190, -1, 300, 20, cos(radians(owner.angle+190))*forceAmount*0, sin(radians(owner.angle+190))*forceAmount*0, int(damage*damageFactor+damageMod*0.3), false));
  ...
  @Override
    ...
    if ((!reverse || owner.reverseImmunity)&&(!freeze || owner.freezeImmunity) &&  active && !owner.dead) {
      ...
      if (shields.size()<3) {
        if (freeze ) {
          shields.add( new Shield( owner, int( owner.cx+cos(radians(owner.angle+60))*shieldRange), int(owner.cy+sin(radians(owner.angle+60))*shieldRange), owner.playerColor, 10000, owner.angle-90+60, damage).setSize(shieldSize));
          shields.add( new Shield( owner, int( owner.cx+cos(radians(owner.angle+30))*shieldRange), int(owner.cy+sin(radians(owner.angle+30))*shieldRange), owner.playerColor, 10000, owner.angle-90+30, damage).setSize(shieldSize));
          shields.add( new Shield( owner, int( owner.cx+cos(radians(owner.angle))*shieldRange), int(owner.cy+sin(radians(owner.angle))*shieldRange), owner.playerColor, 10000, owner.angle-90, damage).setSize(shieldSize));
          shields.add( new Shield( owner, int( owner.cx+cos(radians(owner.angle-30))*shieldRange), int(owner.cy+sin(radians(owner.angle-30))*shieldRange), owner.playerColor, 10000, owner.angle-90-30, damage).setSize(shieldSize));
          shields.add( new Shield( owner, int( owner.cx+cos(radians(owner.angle-60))*shieldRange), int(owner.cy+sin(radians(owner.angle-60))*shieldRange), owner.playerColor, 10000, owner.angle-90-60, damage).setSize(shieldSize));
        } else {
          shields.add( new Shield( owner, int( owner.cx+cos(radians(owner.angle+60))*shieldRange), int(owner.cy+sin(radians(owner.angle+60))*shieldRange), owner.playerColor, 10000, owner.angle-90+60, damage, int(cos(radians(owner.angle+60))*125), int(sin(radians(owner.angle+60))*125)).setSize(shieldSize));
          shields.add( new Shield( owner, int( owner.cx+cos(radians(owner.angle+30))*shieldRange), int(owner.cy+sin(radians(owner.angle+30))*shieldRange), owner.playerColor, 10000, owner.angle-90+30, damage, int(cos(radians(owner.angle+30))*125), int(sin(radians(owner.angle+30))*125)).setSize(shieldSize));
          shields.add( new Shield( owner, int( owner.cx+cos(radians(owner.angle))*shieldRange), int(owner.cy+sin(radians(owner.angle))*shieldRange), owner.playerColor, 10000, owner.angle-90, damage, int(cos(radians(owner.angle))*125), int(sin(radians(owner.angle))*125)).setSize(shieldSize));
          shields.add( new Shield( owner, int( owner.cx+cos(radians(owner.angle-30))*shieldRange), int(owner.cy+sin(radians(owner.angle-30))*shieldRange), owner.playerColor, 10000, owner.angle-90-30, damage, int(cos(radians(owner.angle-30))*125), int(sin(radians(owner.angle-30))*125)).setSize(shieldSize));
          shields.add( new Shield( owner, int( owner.cx+cos(radians(owner.angle-60))*shieldRange), int(owner.cy+sin(radians(owner.angle-60))*shieldRange), owner.playerColor, 10000, owner.angle-90-60, damage, int(cos(radians(owner.angle-60))*125), int(sin(radians(owner.angle-60))*125)).setSize(shieldSize));
        }
...
class Multiply2 extends Ability {//---------------------------------------------------    Multiply   ---------------------------------
  int range=playerSize, cloneDamage=3, dir;
  final float MAX_FORCE=85, damageFactor=0.3, damage=25;
  boolean maxed;
  float ChargeRate=0.85, restForce;

  float cloneHealthPercent=0.5;
  float forceAmount=0, MODIFIED_MAX_ACCEL=0.003, MODIFIED_ANGLE_FACTOR=0.0;
  ...
  @Override
    ...
    // stamps.add( new ControlStamp(owner.index, int(owner.x), int(owner.y), owner.vx, owner.vy, owner.ax, owner.ay));
    try {
      projectiles.add( new SniperBullet(owner, int( owner.cx+cos(radians(owner.angle))*400), int(owner.cy+sin(radians(owner.angle))*400), 50, owner.playerColor, 10, owner.angle, int(damage*1)));
...
class SplitShot extends Ability {//---------------------------------------------------    SplitShot   ---------------------------------
  //boolean charging;
  final float MAX_FORCE=40, damageFactor=0.3, damage=25;
  float forceAmount=0, MODIFIED_MAX_ACCEL=0.043, MODIFIED_ANGLE_FACTOR=0.0, diffAngle, splitAngle;
  //ANGLE_FACTOR=DEFAULT_ANGLE_FACTOR
  float ChargeRate=1.2, restForce, range=400;
  boolean maxed, timerActive=false;
  int dir ;
  float cloneHealthPercent=0.1, timer;
  ArrayList<Player> cloneList= new ArrayList<Player>();
  Player currentClone;
  ...
  @Override
    ...
    if ((!reverse || owner.reverseImmunity ) ) {
      if (!owner.dead && (!freeze || owner.freezeImmunity)&& active && channeling) {
        ...
        stamps.add( new AbilityStamp(this));
        particles.add(new ShockWave(int( owner.cx+cos(radians(owner.angle))*owner.w), int(owner.cy+sin(radians(owner.angle))*owner.w), 25, 20, 400, owner.playerColor));
        primeImage=new   AfterImage(int(owner.cx+cos(radians(splitAngle+90))*forceAmount*2), int( owner.cy+sin(radians(splitAngle+90))*forceAmount*2), cos(radians(splitAngle+90))*forceAmount*0.7, sin(radians(splitAngle+90))*forceAmount*0.7, splitAngle, 0, 0, 400, 3000, owner.playerColor, owner);
        primeImage.type=3;
        particles.add(primeImage);
        if (maxed) { 
          Buff b=  new Cold(owner, 4000, 0.2);
          b.type=BuffType.ONCE;
          projectiles.add( new SniperBullet(owner, int( owner.cx+cos(radians(owner.angle))*100), int(owner.cy+sin(radians(owner.angle))*100), 50, owner.playerColor, 10, owner.angle, int(damage*0.75)).addBuff(b) );
          timerActive=true;
          timer=0;
        } else {
          owner.MAX_ACCEL=owner.DEFAULT_MAX_ACCEL;
        ...
        owner.pushForce(forceAmount*0.7, owner.keyAngle);
  ...
  @Override
    ...
    if (timerActive) {
      ...
      if (timer>60) {
        ...
        timerActive=false;
        particles.add(new   AfterImage(int( primeImage.x), int( primeImage.y), 0, 0, owner.keyAngle, 0, 0, 400, 2000, owner.playerColor, owner) );
...
class ChargeSlash extends Ability {//---------------------------------------------------    ChargeSlash   ---------------------------------
  ...
  @Override
    ...
    particles.add(new  Gradient(  1000, int(owner.cx +cos(radians(owner.angle))*owner.radius), int(owner.cy+sin(radians(owner.angle))*owner.radius), 0, 0, int(forceAmount*11), int(owner.radius*2), 8, owner.angle, owner.playerColor));
    projectiles.add(new Slash(owner, int(owner.cx), int(owner.cy), 150, WHITE, int(3.3*forceAmount), owner.angle, 50, 1.5*forceAmount+50, cos(radians(owner.angle))*forceAmount*.5, sin(radians(owner.angle))*forceAmount*.3, int(forceAmount*damageFactor)+int(damageMod*.4), true).hitOnce(true));
    projectiles.add(new Slash(owner, int(owner.cx), int(owner.cy), 150, WHITE, 280, owner.angle, -20, 70, cos(radians(owner.angle))*forceAmount, sin(radians(owner.angle))*forceAmount, int(forceAmount*damageFactor*2)+int(damageMod*4), false).hitOnce(true));
