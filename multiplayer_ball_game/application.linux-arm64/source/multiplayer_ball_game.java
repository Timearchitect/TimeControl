...
abstract class Ability implements Cloneable {
  ...
  public void assambleTooltip(String _usetype) {
    useType=_usetype;
...
class RapidFire extends Ability {//---------------------------------------------------    RapidFire   ---------------------------------
  ...
  public @Override
    ...
    if ((!reverse || owner.reverseImmunity) &&  active && !owner.dead &&(stampTime-PastTime) >= Interval) {
      ...
      projectiles.add( new Needle(owner, PApplet.parseInt( owner.cx+cos(radians(owner.angle))*owner.w), PApplet.parseInt(owner.cy+sin(radians(owner.angle))*owner.w), 60, owner.playerColor, 800, owner.angle+InAccurateAngle, cos(radians(owner.angle+InAccurateAngle))*36, sin(radians(owner.angle+InAccurateAngle))*36, projectileDamage).addBuff(new Enlarge(owner, 3000, 4)).addBuff(new Burn(owner, 200, 1, 50)).addBuff(new CriticalHit(owner, critChance+criticalChanceMod, (critDamage+criticalDamageMod))));
  ...
  public void release() {
    if ((!reverse || owner.reverseImmunity ) ) {
      ...
      if (!owner.dead && (!freeze || owner.freezeImmunity)&& active && channeling) {
        ...
        projectiles.add( new SniperBullet(owner, PApplet.parseInt( owner.cx+cos(radians(tempA+owner.angle))*nullRange), PApplet.parseInt(owner.cy+sin(radians(tempA+owner.angle))*nullRange), 50, owner.playerColor, 10, owner.angle+tempA, PApplet.parseInt(projectileDamage-inAccurateAngle*2)).addBuff(new CriticalHit(owner, critChance+criticalChanceMod, (critDamage+criticalDamageMod))));
        owner.angle=tempA;
        owner.pushForce(-12, owner.angle);
        shakeTimer+=15; 
  ...
  public void release() {
    if ((!reverse || owner.reverseImmunity ) ) {
      ...
      if (!owner.dead && (!freeze || owner.freezeImmunity)&& active && channeling) {
        ...
        Spike arrow= new Spike(owner, PApplet.parseInt( owner.cx+cos(radians(owner.angle))*nullRange), PApplet.parseInt(owner.cy+sin(radians(owner.angle))*nullRange), 55, owner.playerColor, 800, owner.angle+sin(radians(count))*inAccurateAngle, cos(radians(owner.angle+sin(radians(count))*inAccurateAngle))*12*sutainCount, sin(radians(owner.angle+sin(radians(count))*inAccurateAngle))*12*sutainCount, PApplet.parseInt((projectileDamage-inAccurateAngle+damageMod*.2f)*sutainCount*.1f));
        arrow.size=90;
    ...
    if ((!reverse || owner.reverseImmunity ) ) {
      if (!owner.dead && (!freeze || owner.freezeImmunity)&& active && channeling) {
        ...
        for (Player t : targets) {
          ...
          HomingMissile p=new HomingMissile(owner, PApplet.parseInt( owner.cx+cos(radians(tempAngle))*50), PApplet.parseInt(owner.cy+sin(radians(tempAngle))*50), 60+30*targets.size(), owner.playerColor, 1400, owner.angle, cos(radians(owner.angle))*30, sin(radians(owner.angle))*30, PApplet.parseInt(damage+damageMod*.7f+(15+damageMod*.5f)*targets.size()));
          p.angle=tempAngle;
          p.locking();  
...
class FlameThrower extends Ability {//---------------------------------------------------    FlameThrower   ---------------------------------
  ...
  int alt, count;
  float sutainCount, MAX_sutainCount=50, accuracy, MODIFIED_ANGLE_FACTOR=0.7f, MODIFIED_MAX_ACCEL=0.1f, projectileDamage;
  FlameThrower() {
    super();
    icon=icons[55];
  ...
  public void press() {
    ...
    if (!active) {
      active=true;
      projectiles.add( new  Blast(owner, PApplet.parseInt( owner.cx), PApplet.parseInt(owner.cy), 0, 150, owner.playerColor, 200, owner.angle, projectileDamage));
    }
  }
  public void action() {
    if ((!reverse || owner.reverseImmunity) &&  active && !owner.dead ) {
      ...
      if (sutainCount%2<0.5f) {
        projectiles.add( new  Blast(owner, PApplet.parseInt( owner.cx), PApplet.parseInt(owner.cy), 1+sutainCount, 45, owner.playerColor, 170, owner.angle+InAccurateAngle, PApplet.parseInt(projectileDamage+sutainCount*.01f)).addBuff(new Burn( owner, 2000, 0.00005f, PApplet.parseInt(random(500, 2000)))));
...
class SneakBall extends Ability {//---------------------------------------------------    SneakBall  ---------------------------------
  ...
  public @Override
    ...
    for (int i=0; i<8; i++) {
      payload[i]= new HomingMissile(owner, PApplet.parseInt( cos(radians(i*45))*100), PApplet.parseInt(sin(radians(i*45))*100), 65, owner.playerColor, 4000, owner.angle, cos(radians(owner.angle))*shootSpeed*.1f, sin(radians(owner.angle))*shootSpeed*.1f, PApplet.parseInt(damage+damageMod*.4f)).parent(ball); 
    ...
    payload[8] =new Bomb(owner, 0, 0, 40, owner.playerColor, 1, owner.angle, 0, 0, PApplet.parseInt(damage+damageMod*.2f), false).parent(ball);
...
class Ravine extends Ability {//---------------------------------------------------    Shotgun   ---------------------------------
  ...
  public void passive() {
    ...
    if (charging && startTime+delay<stampTime) {
      ...
      for (int i=2; i<9; i++) 
        players.add(new Block(players.size(), AI, PApplet.parseInt(owner.cx+cos(radians(owner.angle))*i*100)-40, PApplet.parseInt(owner.cy+sin(radians(owner.angle))*i*100)-40, 80, 80, 90));
      shakeTimer+=10;
      owner.halt();
      charging=false;
      shake=false;
...
class Stun extends Buff {
  ...
  public void update() {
    ...
    if (!freeze || owner.freezeImmunity) count+=.4f;
    strokeWeight(30);
  ...
  public @Override
    ...
    if (!dead && !freeze) { 
      if (reverse) {
        ...
        particles.add(new Particle(PApplet.parseInt(x), PApplet.parseInt(y), 0, 0, 100-PApplet.parseInt(abs(vx)+abs(vy)), 1000, BLACK));
