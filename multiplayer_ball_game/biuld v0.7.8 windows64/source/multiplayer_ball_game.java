...
final float flashAmount=0.5f, shakeAmount=0.5f;
...
class Torpedo extends Ability {//---------------------------------------------------    Torpedo   ---------------------------------
  ...
  public @Override
    void action() {
    stamps.add( new AbilityStamp(this));
    RCRocket RC= new RCRocket(owner, PApplet.parseInt( owner.cx), PApplet.parseInt(owner.cy), 70, owner.playerColor, 2000, owner.angle, 0, cos(radians(owner.angle))*0, sin(radians(owner.angle))*0, damage, true, false);
    RC.blastRadius=300;
...
class Revolver extends Ability {//---------------------------------------------------    Revolver   ---------------------------------
  final int damage=40, angleRecoil=180;
  int r;
...
class Shotgun extends Ability {//---------------------------------------------------    Shotgun   ---------------------------------
  ...
  public void passive() {
    if (charging && (timer+delay/S/F)<millis()) {
      ...
      //particles.add(new Flash(50, 6, WHITE)); 
      projectiles.add( new Bomb(owner, PApplet.parseInt( owner.cx), PApplet.parseInt(owner.cy), 40, owner.playerColor, 1, owner.angle, cos(radians(owner.angle))*20, sin(radians(owner.angle))*20, PApplet.parseInt(damage*0.3f), false));
      projectiles.add( new  Blast(owner, PApplet.parseInt( owner.cx), PApplet.parseInt(owner.cy), 50, 100, owner.playerColor, 200, owner.angle, damage*0.2f));
      projectiles.add( new  Blast(owner, PApplet.parseInt( owner.cx), PApplet.parseInt(owner.cy), 20, 200, owner.playerColor, 200, owner.angle, damage*0.2f));
...
class ElemetalLauncher extends Ability {//---------------------------------------------------    ElemetalLauncher   ---------------------------------
  ...
  public @Override
    ...
    case 1:
      Container waterRocket= new Rocket(owner, PApplet.parseInt( owner.cx), PApplet.parseInt(owner.cy), 50, owner.playerColor, 800, owner.angle, cos(radians(owner.angle))*shootSpeed+owner.vx, sin(radians(owner.angle))*shootSpeed+owner.vy, damage, false);
...
class SerpentFire extends Ability {//---------------------------------------------------    RapidFire   ---------------------------------
  ...
  public @Override
    ...
    if ((!reverse || owner.reverseImmunity) &&  active && !owner.dead &&(stampTime-PastTime) >= interval) {
      ...
      SinRocket sr= new SinRocket(owner, PApplet.parseInt( owner.cx), PApplet.parseInt(owner.cy), 25, owner.playerColor, 2000, owner.angle, cos(radians(owner.angle))*shootSpeed*.01f+owner.vx, sin(radians(owner.angle))*shootSpeed*.01f+owner.vy, projectileDamage*2, false);
...
class ThrowBoomerang extends Ability {//---------------------------------------------------    Boomerang   ---------------------------------
  ...
  int damage=3, projectileSize=60;
  PShape boomerang;
  ThrowBoomerang() {
    super();
    icon=icons[18];
...
class DeployThunder extends TimeBomb {//---------------------------------------------------    DeployThunder   ---------------------------------
  ...
  public @Override
    ...
    if (energy>=maxEnergy-activeCost) {   
      ...
      projectiles.add( new Thunder(owner, PApplet.parseInt( owner.cx), PApplet.parseInt(owner.cy), 500, owner.playerColor, 3000, owner.angle, cos(radians(owner.angle))*shootSpeed, sin(radians(owner.angle))*shootSpeed, PApplet.parseInt(damage*1.2f), 6, true));
    } else {
...
class SneakBall extends Ability {//---------------------------------------------------    SneakBall  ---------------------------------
  ...
  public @Override
    ...
    for (int i=0; i<8; i++) {
      payload[i]= new HomingMissile(owner, PApplet.parseInt( cos(radians(i*45))*100), PApplet.parseInt(sin(radians(i*45))*100), 50, owner.playerColor, 4000, owner.angle, cos(radians(owner.angle))*shootSpeed*.1f, sin(radians(owner.angle))*shootSpeed*.1f, damage).parent(ball); 
...
class TripleShot extends Ability {//---------------------------------------------------    DeployDrone  ---------------------------------
  ...
  public @Override
    ...
    particles.add(new ShockWave(PApplet.parseInt(owner.cx), PApplet.parseInt(owner.cy), 15, 16, 600, owner.playerColor));
  ...
  public @Override
    ...
    if ((!reverse || owner.reverseImmunity)&&  owner.health<=0 && (!freeze || owner.freezeImmunity)) {
     ...
     coins+=bonus;
      ...
      particles.add( new Text("+"+bonus, PApplet.parseInt( owner.cx), PApplet.parseInt(owner.cy), 0, 0, 150, 0, 2000, color(50,255,255), 0));
    }
  }
}
...
class Boost extends Ability {//---------------------------------------------------    bullet   ---------------------------------
  ...
  public @Override
    ...
    if (charge>maxCharge && cooldown>60) {
      cooldown=0;
      charge=0;
      projectiles.add( new  Blast(owner, PApplet.parseInt( owner.cx), PApplet.parseInt(owner.cy), 0, 60, owner.playerColor, 350, 0, 1, 10));

      owner.pushForce(45, owner.keyAngle);
      projectiles.add( new  Blast(owner, PApplet.parseInt( owner.cx), PApplet.parseInt(owner.cy), 10, 20, owner.playerColor, 450, owner.keyAngle, 1, 10));
    }
...
class AbilityPack extends Projectile implements Containable {//----------------------------------------- AbilityPack objects ----------------------------------------------------
  ...
  public void display() {
    if (!dead) { 
      ...
      if (adding)rect( x-size*.5f, y-size*.5f, size+20*sin(radians(count)), size+20*cos(radians(count)));
      strokeWeight(PApplet.parseInt(sin(radians(angle*30))*10+10));
...
public void spawningSetup() {
  ...
  spawnList.add(new Spawner(new Object[]{new AbilityPack(AI, new Random().randomize(abilityList), PApplet.parseInt( AI.cx+cos(radians(AI.angle))*AI.w*2), PApplet.parseInt(AI.cy+sin(radians(AI.angle))*AI.w*2), 100, AI.playerColor, 30000, 0, 0, 0, false, true)}
    , 3000, 0, true, 1));
  spawnList.add(new Spawner(new Object[]{new AbilityPack(AI, new Random().randomize(abilityList), PApplet.parseInt( AI.cx+cos(radians(AI.angle))*AI.w*2), PApplet.parseInt(AI.cy+sin(radians(AI.angle))*AI.w*2), 100, AI.playerColor, 30000, 0, 0, 0, false, true)}
    , 3000, 0, true, 1));
  spawnList.add(new Spawner(new Object[]{new AbilityPack(AI, new Random().randomize(abilityList), PApplet.parseInt( AI.cx+cos(radians(AI.angle))*AI.w*2), PApplet.parseInt(AI.cy+sin(radians(AI.angle))*AI.w*2), 100, AI.playerColor, 30000, 0, 0, 0, false, true)}
  ...
  spawnList.add(new Spawner(new Object[]{new AbilityPack(AI, new Random().randomize(passiveList), PApplet.parseInt( AI.cx+cos(radians(AI.angle))*AI.w*2), PApplet.parseInt(AI.cy+sin(radians(AI.angle))*AI.w*2), 100, AI.playerColor, 30000, 0, 0, 0, false, true)}
    , 3000, 0, true, 1));
  spawnList.add(new Spawner(new Object[]{new AbilityPack(AI, new Random().randomize(passiveList), PApplet.parseInt( AI.cx+cos(radians(AI.angle))*AI.w*2), PApplet.parseInt(AI.cy+sin(radians(AI.angle))*AI.w*2), 100, AI.playerColor, 30000, 0, 0, 0, false, true)}
    , 3000, 0, true, 1));
  spawnList.add(new Spawner(new Object[]{new AbilityPack(AI, new Random().randomize(passiveList), PApplet.parseInt( AI.cx+cos(radians(AI.angle))*AI.w*2), PApplet.parseInt(AI.cy+sin(radians(AI.angle))*AI.w*2), 100, AI.playerColor, 30000, 0, 0, 0, false, true)}
  ...
  spawnList.add(new Spawner(new Object[]{new AbilityPack(AI, new Random().randomize(passiveList), PApplet.parseInt( AI.cx+cos(radians(AI.angle))*AI.w*2), PApplet.parseInt(AI.cy+sin(radians(AI.angle))*AI.w*2), 100, AI.playerColor, 30000, 0, 0, 0, true, true)}
    , 5000, 0, true, 1));
  spawnList.add(new Spawner(new Object[]{new AbilityPack(AI, new Random().randomize(passiveList), PApplet.parseInt( AI.cx+cos(radians(AI.angle))*AI.w*2), PApplet.parseInt(AI.cy+sin(radians(AI.angle))*AI.w*2), 100, AI.playerColor, 30000, 0, 0, 0, true, true)}
    , 5000, 0, true, 1));
  spawnList.add(new Spawner(new Object[]{new AbilityPack(AI, new Random().randomize(abilityList), PApplet.parseInt( AI.cx+cos(radians(AI.angle))*AI.w*2), PApplet.parseInt(AI.cy+sin(radians(AI.angle))*AI.w*2), 100, AI.playerColor, 30000, 0, 0, 0, true, true)}
    , 5000, 0, true, 1));
  spawnList.add(new Spawner(new Object[]{new AbilityPack(AI, new Random().randomize(passiveList), PApplet.parseInt( AI.cx+cos(radians(AI.angle))*AI.w*2), PApplet.parseInt(AI.cy+sin(radians(AI.angle))*AI.w*2), 100, AI.playerColor, 30000, 0, 0, 0, true, true)}
    , 5000, 0, true, 1));
  spawnList.add(new Spawner(new Object[]{new AbilityPack(AI, new Random().randomize(abilityList), PApplet.parseInt( AI.cx+cos(radians(AI.angle))*AI.w*2), PApplet.parseInt(AI.cy+sin(radians(AI.angle))*AI.w*2), 100, AI.playerColor, 30000, 0, 0, 0, true, true)}
  ...
  spawnList.add(new Spawner(new Object[]{new FollowDrone(players.size(), halfWidth, halfHeight, PApplet.parseInt(playerSize*0.75f), PApplet.parseInt(playerSize*0.75f), 10, 100, 1, new AutoGun(), new  Reward(2))}
    , 20000, 4000/DIFFICULTY_LEVEL, true, 10*DIFFICULTY_LEVEL));

  spawnList.add(new Spawner(new Object[]{new Drone(players.size(), halfWidth, halfHeight, PApplet.parseInt(playerSize*0.75f), PApplet.parseInt(playerSize*0.75f), 10, 200, new AutoGun(), new  Reward(2))}
  ...
  spawnList.add(new Spawner(new Object[]{new FollowDrone(players.size(), halfWidth, halfHeight, PApplet.parseInt(playerSize*0.75f), PApplet.parseInt(playerSize*0.5f), 10, 50, 1, new Pistol(), new  Reward(4))}
  ...
  spawnList.add(new Spawner(new Object[]{new FollowDrone(players.size(), halfWidth, halfHeight, PApplet.parseInt(playerSize*0.75f), PApplet.parseInt(playerSize*0.75f), 10, 120, 1, new SemiAuto(), new  Reward(5))}
    , 130000, 10000/DIFFICULTY_LEVEL, true, 5*DIFFICULTY_LEVEL));
  spawnList.add(new Spawner(new Object[]{new FollowDrone(players.size(), halfWidth, halfHeight, PApplet.parseInt(playerSize*0.75f), PApplet.parseInt(playerSize*0.75f), 10, 120, 1, new Pistol(), new  Reward(4))}
    , 150000, 10000/DIFFICULTY_LEVEL, true, 5*DIFFICULTY_LEVEL));

  spawnList.add(new Spawner(new Object[]{new FollowDrone(players.size(), halfWidth, halfHeight, PApplet.parseInt(playerSize*1), PApplet.parseInt(playerSize*1), 20, 240, 1, new Torpedo(), new  Reward(5))}
  ...
  spawnList.add(new Spawner(new Object[]{new FollowDrone(players.size(), halfWidth, halfHeight, PApplet.parseInt(playerSize*1.75f), PApplet.parseInt(playerSize*1.75f), 15, PApplet.parseInt(400*DIFFICULTY_LEVEL), 1, new Torpedo(), new Torpedo(), new MissleLauncher(), new  Reward(10))}
...
public void generateRandomAbilities(int index, Ability[] list) {
  for (Player p : players) {      
    if (p!=AI && !p.clone &&  !p.turret) {  // no turret or clone weapon switch
      p.abilityList.get(index).reset();
...
public void shopUpdate() {
  ...
  if (selectedAbility!=null) { 
    ...
    rectMode(CENTER);
    if (mouseX>width/2-900*.5f&&width/2+900*.5f>mouseX&&mouseY>height-150-200*.5f&&height-150+200*.5f>mouseY) {
      fill(90, 255, 255);
