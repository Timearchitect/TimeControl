...
final float flashAmount=0.2f, shakeAmount=0.1f, effectVolume=0.012f;
...
int playerControl[][]= {
  ...
  , { PApplet.parseInt('w')-32, PApplet.parseInt('s')-32, PApplet.parseInt('a')-32, PApplet.parseInt('d')-32, PApplet.parseInt('e')-32 }
...
class ThrowDagger extends Ability {//---------------------------------------------------    ThrowDagger   ---------------------------------
  int damage=20, slashdamage2=1, threashold =3;
  ...
  public @Override
    ...
    if (abs(owner.vx)>threashold  ||  abs(owner.vy)>threashold) {
      if (abs(owner.keyAngle-owner.angle)<5) {
        ...
        if (alternate) {
          projectiles.add( new ArchingIceDagger(owner, PApplet.parseInt( owner.cx), PApplet.parseInt(owner.cy), 30, owner.playerColor, 1000, owner.keyAngle, (owner.keyAngle-owner.angle)*6, owner.ax*24, owner.ay*24, PApplet.parseInt(damage+damageMod*.3f)));
      ...
      owner.pushForce(-22, owner.angle);
      ...
      //}
      projectiles.add( new ArchingIceDagger(owner, PApplet.parseInt( owner.cx+cos(radians(owner.angle))*-55), PApplet.parseInt(owner.cy+sin(radians(owner.angle))*-55), 30, owner.playerColor, 1000, owner.angle, owner.angle, sin(radians(owner.angle+140))*24, -cos(radians(owner.angle+140))*24, PApplet.parseInt(damage+damageMod*.3f)*2).addBuff(new Cold(owner, 3000)));
      projectiles.add( new ArchingIceDagger(owner, PApplet.parseInt( owner.cx+cos(radians(owner.angle))*-55), PApplet.parseInt(owner.cy+sin(radians(owner.angle))*-55), 30, owner.playerColor, 1000, owner.angle, owner.angle+180, sin(radians(owner.angle+40))*24, -cos(radians(owner.angle+40))*24, PApplet.parseInt(damage+damageMod*.3f)*2).addBuff(new Cold(owner, 3000)));

      projectiles.add( new ArchingIceDagger(owner, PApplet.parseInt( owner.cx+cos(radians(owner.angle))*-125), PApplet.parseInt(owner.cy+sin(radians(owner.angle))*-125), 30, owner.playerColor, 1000, owner.angle, owner.angle, sin(radians(owner.angle+140))*20, -cos(radians(owner.angle+140))*20, PApplet.parseInt(damage+damageMod*.3f)*2).addBuff(new Cold(owner, 3000)));
      projectiles.add( new ArchingIceDagger(owner, PApplet.parseInt( owner.cx+cos(radians(owner.angle))*-125), PApplet.parseInt(owner.cy+sin(radians(owner.angle))*-125), 30, owner.playerColor, 1000, owner.angle, owner.angle+180, sin(radians(owner.angle+40))*20, -cos(radians(owner.angle+40))*20, PApplet.parseInt(damage+damageMod*.3f)*2).addBuff(new Cold(owner, 3000)));
      projectiles.add( new Slash(owner, PApplet.parseInt( owner.cx), PApplet.parseInt(owner.cy), 32, owner.playerColor, PApplet.parseInt(slashDuration), owner.angle+90, 22, slashRange, sin(owner.keyAngle)*5, cos(owner.keyAngle)*5, slashdamage, true));
      projectiles.add( new Slash(owner, PApplet.parseInt( owner.cx), PApplet.parseInt(owner.cy), 32, owner.playerColor, PApplet.parseInt(slashDuration), owner.angle-90, -22, slashRange, sin(owner.keyAngle)*5, cos(owner.keyAngle)*5, slashdamage, true));
      particles.add( new Feather(100, PApplet.parseInt(owner.cx+cos(radians(owner.keyAngle-45))*75), PApplet.parseInt(owner.cy+sin(radians(owner.keyAngle-45))*75), owner.ax*10, owner.ay*10, -10, owner.playerColor));
      particles.add( new Feather(100, PApplet.parseInt(owner.cx+cos(radians(owner.keyAngle+45))*75), PApplet.parseInt(owner.cy+sin(radians(owner.keyAngle+45))*75), owner.ax*10, owner.ay*10, 40, owner.playerColor));
    }
  ...
  public void reset() {
    super.reset();
    cooldownTimer = PApplet.parseInt(cooldownTimer-attackSpeedMod*1.8f);
  }
}
...
class Pistol extends Ability implements AmmoBased {//---------------------------------------------------    Pistol   ---------------------------------
  ...
  public @Override
    ...
    owner.addBuff(new SpeedBuff(owner, 1000, 0.12f).apply(BuffType.ONCE));
    owner.addBuff(new DamageBuff(owner, 2000, PApplet.parseInt(damage*0.5f)).apply(BuffType.ONCE));
...
class Revolver extends Ability implements AmmoBased {//---------------------------------------------------    Revolver   ---------------------------------
  ...
  public void reset() {
    ...
    ammo=0;
    cooldown=0;
    cooldownTimer=PApplet.parseInt(250-attackSpeedMod*2.6f); //100
  }
}
  ...
  public @Override
    ...
    if ((!reverse || owner.reverseImmunity ) ) {
      if (!owner.dead && (!freeze || owner.freezeImmunity)&& active && channeling) {
        //stamps.add( new AbilityStamp(owner.index, int(owner.x), int(owner.y), energy, active, channeling, cooling, regen, hold));
        stamps.add( new AbilityStamp(this));
        particles.add(new ShockWave(PApplet.parseInt(owner.cx+cos(radians(owner.angle))*75), PApplet.parseInt(owner.cy+sin(radians(owner.angle))*75), PApplet.parseInt(forceAmount), 32, PApplet.parseInt(forceAmount*2), owner.playerColor));
...
class DoubleTap extends Laser {//---------------------------------------------------    DoubleTap Ability  ---------------------------------
  ...
  public void passive() {
    ...
    play(thumpSound);
      ...
      switch(chargelevel) {
      case 1:
        projectiles.add( new Slug(owner, PApplet.parseInt( owner.cx+cos(radians(owner.angle))*owner.w), PApplet.parseInt(owner.cy+sin(radians(owner.angle))*owner.w), 40, owner.playerColor, 1200, owner.angle, cos(radians(owner.angle))*26, sin(radians(owner.angle))*26, PApplet.parseInt(damage+damageMod)));
        ...
        break;
      case 2:
        projectiles.add( new Slug(owner, PApplet.parseInt( owner.cx+cos(radians(owner.angle))*owner.w), PApplet.parseInt(owner.cy+sin(radians(owner.angle))*owner.w), 160, owner.playerColor, 10000, owner.angle, cos(radians(owner.angle))*12, sin(radians(owner.angle))*12, PApplet.parseInt(damage*3+damageMod*4)));
        ...
        particles.add(new Flash(50, 6, WHITE)); 
        projectiles.add( new  Blast(owner, PApplet.parseInt( owner.cx), PApplet.parseInt(owner.cy), 20, 300, owner.playerColor, 200, owner.angle, damage*0.1f));
        break;
      }    
...
class Sluggun extends Ability {//---------------------------------------------------    Shotgun   ---------------------------------
  ...
  public void passive() {
    if (charging && (timer+delay/S/F)<millis()) {
      ...
      //particles.add(new ShockWave(int(owner.cx), int(owner.cy), 200*chargelevel, 16, 500, owner.playerColor));
      //particles.add(new Flash(50, 6, WHITE)); 
      projectiles.add( new Bomb(owner, PApplet.parseInt( owner.cx), PApplet.parseInt(owner.cy), 40, owner.playerColor, 1, owner.angle, cos(radians(owner.angle))*20, sin(radians(owner.angle))*20, PApplet.parseInt(damage*.2f), false));
      projectiles.add( new  Blast(owner, PApplet.parseInt( owner.cx), PApplet.parseInt(owner.cy), 30, 100, owner.playerColor, 200, owner.angle, damage*.05f));
      projectiles.add( new  Blast(owner, PApplet.parseInt( owner.cx), PApplet.parseInt(owner.cy), 10, 200, owner.playerColor, 200, owner.angle, damage*.05f));
...
class FlashBomb extends Ability {//---------------------------------------------------    TimeBomb   ---------------------------------
  ...
  public @Override
    ...
    projectiles.add( new FlashBang(owner, PApplet.parseInt( owner.cx), PApplet.parseInt(owner.cy), 50, owner.playerColor, PApplet.parseInt(random(500, 2200)), owner.angle, cos(radians(owner.angle))*shootSpeed, sin(radians(owner.angle))*shootSpeed, PApplet.parseInt(damage+damageMod), true));
...
class ElemetalLauncher extends Ability {//---------------------------------------------------    ElemetalLauncher   ---------------------------------
  ...
  public @Override
    ...
    cooldown=stampTime;
    owner.MAX_ACCEL=owner.DEFAULT_MAX_ACCEL;
    cooldownTimer=PApplet.parseInt(800-attackSpeedMod*8); //100
  }
}
...
class Bazooka extends Ability {//---------------------------------------------------    Bazooka   ---------------------------------
  ...
  public @Override
    ...
    case 2:
      ...
      break;
    case 3:
      //  projectiles.add( new Missile(owner, int( owner.cx), int(owner.cy), 35, owner.playerColor, 7000, owner.angle, cos(radians(owner.angle))*shootSpeed, sin(radians(owner.angle))*shootSpeed, damage, false));
      Missile m= new Missile(owner, PApplet.parseInt( owner.cx), PApplet.parseInt(owner.cy), 40, owner.playerColor, 1100+PApplet.parseInt(random(400)), owner.angle, cos(radians(owner.angle))*60, sin(radians(owner.angle))*60, damage, true);
      m.blastRadius=400;
  ...
  public @Override
    ...
    deChannel();
    regen=true;
    cooldownTimer=PApplet.parseInt(1000-attackSpeedMod*10); //100
  }
}

class Stars extends Ability {//---------------------------------------------------    Stars   ---------------------------------
  int  damage=20, shootSpeed=6, size=20;
  ...
  public @Override
    ...
    deChannel();
    regen=true;
    cooldownTimer=PApplet.parseInt(1000-attackSpeedMod*10);
  }
}
...
class RapidFire extends Ability {//---------------------------------------------------    RapidFire   ---------------------------------
  ...
  public void reset() {
    super.reset();
    PastTime=stampTime;
    Interval = PApplet.parseInt(DEFAULT_INTERVAL-attackSpeedMod);
    // channeling=false;
    // active=false;
    // hold=false;
    // deactivate();
...
class CrossFire extends RapidFire {//---------------------------------------------------    CrossFire   ---------------------------------
  float accuracy = 1, MODIFIED_ANGLE_FACTOR=-0.0008f, r=50, maxR=100, damage, crossAngle;  
  int Interval=140;
  long  PastTime;
  //int projectileDamage=5;
  ...
  public @Override
    ...
    if ((!reverse || owner.reverseImmunity) &&  active && !owner.dead &&(stampTime-PastTime) >= Interval) {
      ...
      Projectile p= new Needle(owner, PApplet.parseInt( owner.cx+cos(radians(owner.angle+90))*owner.w), PApplet.parseInt(owner.cy+sin(radians(owner.angle+90))*owner.w), 60, owner.playerColor, 800, owner.angle+InAccurateAngle, cos(radians(owner.angle+InAccurateAngle+crossAngle))*56, sin(radians(owner.angle+InAccurateAngle+crossAngle))*56, PApplet.parseInt(damage)).addBuff(new Burn(owner, 150, 1, 50)).addBuff(new CriticalHit(owner, critChance+criticalChanceMod, (critDamage+criticalDamageMod)));
      Projectile p2= new Needle(owner, PApplet.parseInt( owner.cx+cos(radians(owner.angle-90))*owner.w), PApplet.parseInt(owner.cy+sin(radians(owner.angle-90))*owner.w), 60, owner.playerColor, 800, owner.angle+InAccurateAngle, cos(radians(owner.angle-InAccurateAngle-crossAngle))*56, sin(radians(owner.angle-InAccurateAngle-crossAngle))*56, PApplet.parseInt(damage)).addBuff(new Burn(owner, 150, 1, 50)).addBuff(new CriticalHit(owner, critChance+criticalChanceMod, (critDamage+criticalDamageMod)));
      if (crossAngle<-16) {
        p.ally=-1;
        p2.ally=-1;
      ...
      if (crossAngle<-36) {
        projectiles.add( new Bomb(owner, PApplet.parseInt( owner.cx+cos(radians(owner.angle))*owner.w), PApplet.parseInt(owner.cy+sin(radians(owner.angle))*owner.w), 150, BLACK, 400, owner.angle, 0, 0, PApplet.parseInt(damage*10), true));
        projectiles.add( new ChargeLaser(owner, PApplet.parseInt( owner.cx+cos(radians(owner.angle))*owner.w), PApplet.parseInt(owner.cy+sin(radians(owner.angle))*owner.w), 550, BLACK, 150, owner.angle+40, 1, damage )); 
        projectiles.add( new ChargeLaser(owner, PApplet.parseInt( owner.cx+cos(radians(owner.angle))*owner.w), PApplet.parseInt(owner.cy+sin(radians(owner.angle))*owner.w), 550, BLACK, 150, owner.angle-40, -1, damage )); 
      ...
      projectiles.add(p2);
      crossAngle-=1.7f;
      particles.add(new ShockWave(PApplet.parseInt(owner.cx+cos(radians(owner.angle+90))*owner.w), PApplet.parseInt(owner.cy+sin(radians(owner.angle)+90)*owner.w), 5, 22, 20, WHITE));
      particles.add(new ShockWave(PApplet.parseInt(owner.cx+cos(radians(owner.angle-90))*owner.w), PApplet.parseInt(owner.cy+sin(radians(owner.angle)-90)*owner.w), 5, 22, 20, WHITE));
      owner.ANGLE_FACTOR=MODIFIED_ANGLE_FACTOR;
      r=50;
      channel();
  ...
  public @Override
    ...
    if (!owner.stealth) {
      ...
      strokeWeight(12);
      if (r<maxR)r*=1.1f;
      line(PApplet.parseInt( owner.cx+cos(radians(owner.angle+90))*owner.w), PApplet.parseInt( owner.cy+sin(radians(owner.angle+90))*owner.w), PApplet.parseInt( owner.cx+cos(radians(owner.angle+90))*owner.w*r*.005f), PApplet.parseInt( owner.cy+sin(radians(owner.angle+90))*owner.w*r*.005f));
      line(PApplet.parseInt( owner.cx+cos(radians(owner.angle-90))*owner.w), PApplet.parseInt( owner.cy+sin(radians(owner.angle-90))*owner.w), PApplet.parseInt( owner.cx+cos(radians(owner.angle-90))*owner.w*r*.005f), PApplet.parseInt( owner.cy+sin(radians(owner.angle-90))*owner.w*r*.005f));
    }
  }
  public void reset() {
    super.reset();
    PastTime=stampTime;
    Interval=PApplet.parseInt(140-attackSpeedMod*1.4f); //100
  ...
  public void passive() {
    ...
    if (!owner.stealth) {   
      ...
      if (cooldown<stampTime && active) {
        ...
        //   line(owner.cx+xOffset, owner.cy+yOffset, int( owner.cx+cos(radians(-inAccurateAngle*3.25+owner.angle))*2000), int(owner.cy+sin(radians(-inAccurateAngle*3.25+owner.angle))*2000));
        aimLine(nullRange, 3000, inAccurateAngle*3.25f);
        aimLine(nullRange, 3000, -inAccurateAngle*3.25f);
        noFill();
        ...
        aimLine(nullRange, 3000, inAccurateAngle*0.25f);
        aimLine(nullRange, 3000, -inAccurateAngle*0.25f);
...
class Battery extends Ability {//---------------------------------------------------    Battery  Ability  ---------------------------------
  int  maxInterval=5, DEFAULT_MAX_INTERVAL=5, damage=7, count=0, maxCount=6;
  ...
  Battery() {
    ...
    unlockCost=2000;
    assambleTooltip("Tap");
    maxInterval=PApplet.parseInt(DEFAULT_MAX_INTERVAL-attackSpeedMod*0.04f);
  } 
...
class RapidBattery extends Battery {//---------------------------------------------------    RapidBattery Ability   ---------------------------------
  int  maxInterval=1, damage=2, count=0, maxCount=18;
...
class AssaultBattery extends Ability {//---------------------------------------------------    Battery   ---------------------------------
  int  DEFAULT_MAX_INTERVAL=10, maxInterval=DEFAULT_MAX_INTERVAL, damage=14, count=0, maxCount=13, flip=1;
  ...
  public @Override
    ...
    stroke(owner.playerColor);
    if (flip==1) {
      arc(owner.cx, owner.cy, 2000, 2000, radians(owner.angle-accuracy*1.0f), radians(owner.angle));
    } else {
      arc(owner.cx, owner.cy, 2000, 2000, radians(owner.angle), radians(owner.angle+accuracy*1.0f));
    }
    strokeWeight(1);
    flip*=-1;
...
class MarbleLauncher extends Ability {//---------------------------------------------------    MarbleLauncher   ---------------------------------
  ...
  public @Override
    ...
    case 0:
      ...
      //projectiles.add( new Missile(owner, int( owner.cx+cos(radians(owner.angle+90))*50), int(owner.cy+sin(radians(owner.angle+90))*50), 30, owner.playerColor, duration, owner.angle, cos(radians(owner.angle))*shootSpeed, sin(radians(owner.angle))*shootSpeed, damage, false));
  ...
  public @Override
    ...
    owner.ANGLE_FACTOR=owner.DEFAULT_ANGLE_FACTOR;
    owner.MAX_ACCEL=owner.DEFAULT_MAX_ACCEL;
    cooldownTimer=PApplet.parseInt(1300-attackSpeedMod*13); //100
  }
}
class MissileLauncher extends Ability {//---------------------------------------------------    MissileLauncher   ---------------------------------
  int  maxInterval=6, damage=6, offset=50, accuracy=10, count=0, maxCount=6, shootSpeed=44, duration=4500;
  ...
  public @Override
    ...
    case 0:
      owner.ANGLE_FACTOR= MODIFIED_ANGLE_FACTOR;
      projectiles.add( new Missile(owner, PApplet.parseInt( owner.cx+cos(radians(owner.angle+90))*50), PApplet.parseInt(owner.cy+sin(radians(owner.angle+90))*50), 30, owner.playerColor, duration, owner.angle, cos(radians(owner.angle))*shootSpeed, sin(radians(owner.angle))*shootSpeed, PApplet.parseInt(damage+damageMod*.2f), false).addBuff(new CriticalHit(owner, critChance+criticalChanceMod, (critDamage+criticalDamageMod))));
      particles.add( new  Particle(PApplet.parseInt(owner.cx+cos(radians(owner.angle+90))*50), PApplet.parseInt(owner.cy+sin(radians(owner.angle+90))*50), 0, 0, 50, 50, WHITE));
    ...
    case 3:
      ...
      break;
    case 4:
      projectiles.add( new Missile(owner, PApplet.parseInt( owner.cx+cos(radians(owner.angle+100))*50), PApplet.parseInt(owner.cy+sin(radians(owner.angle+100))*50), 30, owner.playerColor, duration, owner.angle+20, cos(radians(owner.angle+20))*shootSpeed, sin(radians(owner.angle+20))*shootSpeed, PApplet.parseInt(damage+damageMod*.2f), false).addBuff(new CriticalHit(owner, critChance+criticalChanceMod, (critDamage+criticalDamageMod))));
      particles.add( new  Particle(PApplet.parseInt(owner.cx+cos(radians(owner.angle-90))*50), PApplet.parseInt(owner.cy+sin(radians(owner.angle-90))*50), 0, 0, 50, 50, WHITE));
  ...
  public @Override
    ...
    owner.ANGLE_FACTOR=owner.DEFAULT_ANGLE_FACTOR;
    owner.MAX_ACCEL=owner.DEFAULT_MAX_ACCEL;
    cooldownTimer=PApplet.parseInt(2300-attackSpeedMod*20); //100
  }
}
...
class CutThroat extends SeekGun { 
  float expandRate=0.06f;
  int originX, originY, pcx, pcy, targetTurnIndex, maxRange=1800, DEFAULT_MIN_RANGE=40, rangeDiff=20;
  boolean slaughter;
  CutThroat() {
    super();
    icon=icons[23];
  ...
  public void press() {
    if (!owner.dead) {
      owner.pushForce(-42, owner.keyAngle);
      particles.add( new Star(1500, PApplet.parseInt(owner.cx+cos(radians(owner.angle))*-75), PApplet.parseInt(owner.cy+sin(radians(owner.angle))*-75), 0, 0, 400, 0.7f, owner.playerColor) );
    }
    super.press();
  }
        ...
        if (p.targetable) {
          ...
          if (!p.dead && p.ally!=owner.ally && (dist(owner.cx, owner.cy, p.cx, p.cy)<range+p.radius && dist(owner.cx, owner.cy, p.cx, p.cy)>minRange-p.radius
            ...
            if (!targets.contains(p)) {
              ...
              // zoomRate=0.3;
              particles.add(new ShockWave(PApplet.parseInt(p.cx), PApplet.parseInt(p.cy), 50, 32, 200, owner.playerColor));
            }
          }
        }
      }
    ...
    if ((!reverse || owner.reverseImmunity ) ) {
      if (!owner.dead && (!freeze || owner.freezeImmunity)&& active && channeling) {
        ...
        if (targets.size()>0) {
          minAngle=70;
          expandRate=0.06f;
          particles.add(new RShockWave(PApplet.parseInt(owner.cx), PApplet.parseInt(owner.cy), 400, 30, 850, WHITE));
          particles.add( new Star(1800, PApplet.parseInt(owner.cx+cos(radians(owner.angle))*75), PApplet.parseInt(owner.cy+sin(radians(owner.angle))*75), 0, 0, 200+100*targets.size(), 0.7f, owner.playerColor) );
        } else {
          minAngle=30;
          expandRate+=0.02f;
          owner.addBuff(new ArmorPiercing(owner, 5000, 10));
          owner.addBuff(new DamageBuff(owner, 6000, 25));
        }
  ...
  public @Override
    ...
    if (slaughter && !freeze) {
      ...
      if (targetTurnIndex<targets.size()) {   
        ...
        projectiles.add( new Slash(owner, PApplet.parseInt( owner.cx+cos(radians(owner.angle))*65), PApplet.parseInt(owner.cy+sin(radians(owner.angle))*65), 32, owner.playerColor, 130, owner.angle+100, 24, 140, sin(owner.keyAngle)*5, cos(owner.keyAngle)*5, PApplet.parseInt((damage+damageMod*0.1f)*(2.5f-targets.size()*.07f)), false).hitOnce(true).addBuff(new CriticalHit(owner, critChance+criticalChanceMod, (critDamage+criticalDamageMod))));
...
class HitScanGun extends SeekGun {
  ...
  public void reset() {
    super.reset();
    cooldownTimer=PApplet.parseInt(100-attackSpeedMod*1); //100
  }
}
...
class ThrowBoomerang extends Ability {//---------------------------------------------------    Boomerang   ---------------------------------
  ...
  int projectileSize=60, slashdamage=10;
  PShape boomerang;
  ThrowBoomerang() {
    super();
    icon=icons[18];
  ...
  public @Override
    ...
    if ((!reverse || owner.reverseImmunity ) ) {
      if (!owner.dead && (!freeze || owner.freezeImmunity)&& active && channeling) {
        ...
        //stamps.add( new AbilityStamp(owner.index, int(owner.x), int(owner.y), energy, active, channeling, cooling, regen, hold));
        stamps.add( new AbilityStamp(this));
        projectiles.add( new Slash(owner, PApplet.parseInt( owner.cx), PApplet.parseInt(owner.cy), 40, owner.playerColor, 120, owner.angle+100, 24, 80, sin(owner.keyAngle)*5, cos(owner.keyAngle)*5, slashdamage, false));
        regen=true;
        action();
...
class PhotonicPursuit extends Ability {//---------------------------------------------------    PhotonicPursuit   ---------------------------------
  int damage=26, customAngle, initialSpeed=6, r;
    ...
    if (energy>=maxEnergy-15) {
      ...
      for (Player p : players) {
        if (!p.dead && p.targetable) {
          ...
          HomingMissile h= new HomingMissile(owner, PApplet.parseInt( owner.cx), PApplet.parseInt(owner.cy), 70, owner.playerColor, 5000, owner.angle, cos(radians(owner.angle+customAngle))*30, sin(radians(owner.angle+customAngle))*initialSpeed, PApplet.parseInt(damage+damageMod*.4f));
          h.target=p;
          projectiles.add(h.addBuff(new CriticalHit(owner, critChance+criticalChanceMod, (critDamage+criticalDamageMod))));
          customAngle=90;

          h= new HomingMissile(owner, PApplet.parseInt( owner.cx), PApplet.parseInt(owner.cy), 70, owner.playerColor, 5000, owner.angle, cos(radians(owner.angle+customAngle))*30, sin(radians(owner.angle+customAngle))*initialSpeed, PApplet.parseInt(damage+damageMod*.4f));
          h.target=p;
          projectiles.add(h.addBuff(new CriticalHit(owner, critChance+criticalChanceMod, (critDamage+criticalDamageMod))));
          customAngle=0;
        }
...
class DeployShield extends Ability {//---------------------------------------------------    DeployShield   ---------------------------------
  int damage=14, shell=80, pHealth;
  ...
  public void reset() {
    super.reset();
    owner.armor=0;
    cooldownTimer=PApplet.parseInt(2800-attackSpeedMod*28); //100
  }
}
class Explosion extends Ability {//---------------------------------------------------    Explosion   ---------------------------------
  ...
  public @Override
    ...
    Container waterRocket= new Rocket(owner, PApplet.parseInt( owner.cx-cos(radians(owner.angle))*300), PApplet.parseInt(owner.cy-sin(radians(owner.angle))*300), 50, owner.playerColor, 80, owner.angle, cos(radians(owner.angle))*shootSpeed+owner.vx, sin(radians(owner.angle))*shootSpeed+owner.vy, PApplet.parseInt(damage*0.3f), false);
    payload=new Containable[2];
    payload[0]= new ChargeLaser(owner, 0, 0, 650, owner.playerColor, 150, owner.angle-180, 16, damage*.3f ).parent(waterRocket); 
    payload[1]= new ChargeLaser(owner, 0, 0, 650, owner.playerColor, 150, owner.angle+180, -16, damage*.3f ).parent(waterRocket); 
    ...
    owner.stop();
    owner.pushForce(35, owner.angle);
  }
  public @Override
    void press() {
    if (cooldown<stampTime &&(!reverse || owner.reverseImmunity)&& energy>0+activeCost && !owner.dead && (!freeze || owner.freezeImmunity)) {
  ...
  public void shatter() {
    owner.halt();
    for (int i=0; i<10; i++) {
      ChargeLaser c= new ChargeLaser(owner, PApplet.parseInt( owner.cx), PApplet.parseInt(owner.cy), 650, owner.playerColor, 150, i*36, -16, damage*.1f );
      c.laserLength=400;
      projectiles.add(c);
...
class Gravity extends Ability {//---------------------------------------------------    Gravity   ---------------------------------
  ...
  public @Override
    ...
    gravitonList.clear();
    cooldownTimer=PApplet.parseInt(1200-attackSpeedMod*10); //100
  }
  public void passive() {
    if (!owner.stealth) {
...
class DeployTurret extends Ability {//---------------------------------------------------    DeployTurret  ---------------------------------
  ...
  public @Override
    ...
    cooldownTimer=PApplet.parseInt(3000-attackSpeedMod*25); //100
  }
}
class DeployDrone extends Ability {//---------------------------------------------------    DeployDrone  ---------------------------------
  ...
  public @Override
    ...
    cooldownTimer=PApplet.parseInt(2000-attackSpeedMod*20); //100
  }
}
class DeployBodyguard extends DeployDrone {//---------------------------------------------------    DeployDrone  ---------------------------------
  ...
  public @Override
    ...
    super.reset();
    players.remove(droneList);
    cooldownTimer=PApplet.parseInt(2500-attackSpeedMod*20); //100
  }
}
...
class CloudStrike extends Ability {//---------------------------------------------------    CloudStrike   ---------------------------------
  ...
  public void reset() {
    ...
    release();
    forceAmount=0;
    cooldownTimer=PApplet.parseInt(1500-attackSpeedMod*10); //100
  }
  public @Override
    void passive() {
  }
...
class Detonator extends Ability {//---------------------------------------------------    Detonator   ---------------------------------
  ...
  public @Override
    ...
    if ( bomb!= null && !bomb.dead && bomb.deathTime>stampTime) {
      ...
      //bomb.detonate();
            particles.add(new ShockWave(PApplet.parseInt(bomb.x), PApplet.parseInt(bomb.y), 20, 16, 50, owner.playerColor));
...
class RandoGun extends Ability {//---------------------------------------------------    RandoGun   ---------------------------------
  ...
  int shootSpeed=35;
  final String description[]={"dagger", "tesla", "force", "shotgun", "revolver", "Homing missile", "electron", "laser", "bomb", "RC", "Boomerang", "Sniper", "thunder", "cluster", "mine", "Missiles", "rocket", "torpedo", "slice"};
  RandoGun() {
    super();
    //name=getClassName(this);
    activeCost=18;
  ...
  public @Override
    ...
    case 14:
      ...
      break;
    case 15:
      projectiles.add( new Missile(owner, PApplet.parseInt( owner.cx+cos(radians(owner.angle+90))*50), PApplet.parseInt(owner.cy+sin(radians(owner.angle+90))*50), 30, owner.playerColor, 5000, owner.angle, cos(radians(owner.angle-90))*shootSpeed, sin(radians(owner.angle-90))*shootSpeed, damage, false));
      projectiles.add( new Missile(owner, PApplet.parseInt( owner.cx+cos(radians(owner.angle+90))*50), PApplet.parseInt(owner.cy+sin(radians(owner.angle+90))*50), 30, owner.playerColor, 5000, owner.angle, cos(radians(owner.angle+90))*shootSpeed, sin(radians(owner.angle+90))*shootSpeed, damage, false));
      break;
    case 16:
...
class Artilery extends Ability {//---------------------------------------------------    Artilery Ability  ---------------------------------
  ...
  int shootSpeed=40, DEFAULT_MAX_INTERVAL=150;
  int damage=20, duration=200, tDuration, alt=1;
  float r=10, r2=10, maxR=150, MODIFIED_ANGLE_FACTOR = 0.008f, spreadAngle=15, retractFactor=0.95f, extendFactor=1.11f;   
  long timer, tTimer;
  ...
  Artilery() {
    ...
    unlockCost=3600;
    assambleTooltip("Toogle");
    maxR=PApplet.parseInt(DEFAULT_MAX_INTERVAL-attackSpeedMod*0.150f);
    duration=PApplet.parseInt(200-attackSpeedMod*0.200f);
    extendFactor=1.11f+attackSpeedMod*0.01f;
  } 
  ...
  public void passive() {
    if (transformed) {
      ...
      channel();
      owner.ANGLE_FACTOR=MODIFIED_ANGLE_FACTOR;
      if (r<maxR)r*=extendFactor*timeBend;
      if (r2<maxR)r2*=extendFactor*timeBend;
    } else {
      if (r>10)r*=retractFactor*timeBend;
      if (r2>10)r2*=retractFactor*timeBend;
      owner.ANGLE_FACTOR=owner.DEFAULT_ANGLE_FACTOR;
    }
    //  for (HomingMissile m : missileList) 
    // if (!m.leap) m.angle=owner.angle;
...
class LaserSword extends Ability {//---------------------------------------------------    LaserSword Ability ---------------------------------
  ...
  public @Override
    ...
    super.activate();
    transformed=!transformed;
    if (l!=null) l.laserLength=PApplet.parseInt(swordLength*.4f);
...
class StunGun extends Ability {//---------------------------------------------------    StunGun   ---------------------------------
  ...
  public @Override
    ...
    Missile m= new Missile(owner, PApplet.parseInt( owner.cx), PApplet.parseInt(owner.cy), projectileSize, owner.playerColor, 1100+PApplet.parseInt(random(400)), owner.angle, cos(radians(owner.angle))*80, sin(radians(owner.angle))*80, PApplet.parseInt(damage+damageMod*.6f), true);
  ...
  public void reset() {
    super.reset();
    cooldownTimer=PApplet.parseInt(190-attackSpeedMod*1.9f); //100
  }
}
...
class Chivalry extends Ability {
  ...
  int shieldRange=175, shieldSize=44;
  boolean maxed;
  Chivalry() {
    super();
    icon=icons[58];
  ...
  public @Override
    ...
    if ((!reverse || owner.reverseImmunity)&&(!freeze || owner.freezeImmunity) &&  active && !owner.dead) {
      ...
      if (shields.size()<3) {
        if (freeze ) {
          shields.add( new Shield( owner, PApplet.parseInt( owner.cx+cos(radians(owner.angle+60))*shieldRange), PApplet.parseInt(owner.cy+sin(radians(owner.angle+60))*shieldRange), owner.playerColor, 10000, owner.angle-90+60, damage).setSize(shieldSize));
          shields.add( new Shield( owner, PApplet.parseInt( owner.cx+cos(radians(owner.angle+30))*shieldRange), PApplet.parseInt(owner.cy+sin(radians(owner.angle+30))*shieldRange), owner.playerColor, 10000, owner.angle-90+30, damage).setSize(shieldSize));
          shields.add( new Shield( owner, PApplet.parseInt( owner.cx+cos(radians(owner.angle))*shieldRange), PApplet.parseInt(owner.cy+sin(radians(owner.angle))*shieldRange), owner.playerColor, 10000, owner.angle-90, damage).setSize(shieldSize));
          shields.add( new Shield( owner, PApplet.parseInt( owner.cx+cos(radians(owner.angle-30))*shieldRange), PApplet.parseInt(owner.cy+sin(radians(owner.angle-30))*shieldRange), owner.playerColor, 10000, owner.angle-90-30, damage).setSize(shieldSize));
          shields.add( new Shield( owner, PApplet.parseInt( owner.cx+cos(radians(owner.angle-60))*shieldRange), PApplet.parseInt(owner.cy+sin(radians(owner.angle-60))*shieldRange), owner.playerColor, 10000, owner.angle-90-60, damage).setSize(shieldSize));
        } else {
          shields.add( new Shield( owner, PApplet.parseInt( owner.cx+cos(radians(owner.angle+60))*shieldRange), PApplet.parseInt(owner.cy+sin(radians(owner.angle+60))*shieldRange), owner.playerColor, 10000, owner.angle-90+60, damage, PApplet.parseInt(cos(radians(owner.angle+60))*125), PApplet.parseInt(sin(radians(owner.angle+60))*125)).setSize(shieldSize));
          shields.add( new Shield( owner, PApplet.parseInt( owner.cx+cos(radians(owner.angle+30))*shieldRange), PApplet.parseInt(owner.cy+sin(radians(owner.angle+30))*shieldRange), owner.playerColor, 10000, owner.angle-90+30, damage, PApplet.parseInt(cos(radians(owner.angle+30))*125), PApplet.parseInt(sin(radians(owner.angle+30))*125)).setSize(shieldSize));
          shields.add( new Shield( owner, PApplet.parseInt( owner.cx+cos(radians(owner.angle))*shieldRange), PApplet.parseInt(owner.cy+sin(radians(owner.angle))*shieldRange), owner.playerColor, 10000, owner.angle-90, damage, PApplet.parseInt(cos(radians(owner.angle))*125), PApplet.parseInt(sin(radians(owner.angle))*125)).setSize(shieldSize));
          shields.add( new Shield( owner, PApplet.parseInt( owner.cx+cos(radians(owner.angle-30))*shieldRange), PApplet.parseInt(owner.cy+sin(radians(owner.angle-30))*shieldRange), owner.playerColor, 10000, owner.angle-90-30, damage, PApplet.parseInt(cos(radians(owner.angle-30))*125), PApplet.parseInt(sin(radians(owner.angle-30))*125)).setSize(shieldSize));
          shields.add( new Shield( owner, PApplet.parseInt( owner.cx+cos(radians(owner.angle-60))*shieldRange), PApplet.parseInt(owner.cy+sin(radians(owner.angle-60))*shieldRange), owner.playerColor, 10000, owner.angle-90-60, damage, PApplet.parseInt(cos(radians(owner.angle-60))*125), PApplet.parseInt(sin(radians(owner.angle-60))*125)).setSize(shieldSize));
        }
...
class Multiply2 extends Ability {//---------------------------------------------------    Multiply   ---------------------------------
  int range=playerSize, cloneDamage=3, dir;
  final float MAX_FORCE=85, damageFactor=0.3f, damage=25;
  boolean maxed;
  float ChargeRate=0.85f, restForce;

  float cloneHealthPercent=0.5f;
  float forceAmount=0, MODIFIED_MAX_ACCEL=0.003f, MODIFIED_ANGLE_FACTOR=0.0f;
  ...
  public @Override
    ...
    // stamps.add( new ControlStamp(owner.index, int(owner.x), int(owner.y), owner.vx, owner.vy, owner.ax, owner.ay));
    try {
      projectiles.add( new SniperBullet(owner, PApplet.parseInt( owner.cx+cos(radians(owner.angle))*400), PApplet.parseInt(owner.cy+sin(radians(owner.angle))*400), 50, owner.playerColor, 10, owner.angle, PApplet.parseInt(damage*1)));
      ...
      currentClone.replaceAbility(new Multiply2(), new NoActive() );
    }
    catch(Exception e) {
      println(e +"multiply");
    }
  ...
  public @Override
    ...
    if ((!reverse || owner.reverseImmunity ) ) {
      if (!owner.dead && (!freeze || owner.freezeImmunity)&& active && channeling) {
        ...
        owner.pushForce(forceAmount*1, owner.keyAngle);
        regen=true;
        // action();
        enableCooldown();
...
class SplitShot extends Ability {//---------------------------------------------------    SplitShot   ---------------------------------
  //boolean charging;
  final float MAX_FORCE=40, damageFactor=0.3f, damage=25;
  float forceAmount=0, MODIFIED_MAX_ACCEL=0.043f, MODIFIED_ANGLE_FACTOR=0.0f, diffAngle, splitAngle;
  //ANGLE_FACTOR=DEFAULT_ANGLE_FACTOR
  float ChargeRate=1.2f, restForce, range=400;
  boolean maxed, timerActive=false;
  int dir ;
  float cloneHealthPercent=0.1f, timer;
  ArrayList<Player> cloneList= new ArrayList<Player>();
  Player currentClone;
  AfterImage primeImage;
  SplitShot() {
    super();
    icon=icons[40];
    //name=getClassName(this);
  ...
  public @Override
    ...
    if ((!reverse || owner.reverseImmunity ) ) {
      if (!owner.dead && (!freeze || owner.freezeImmunity)&& active && channeling) {
        ...
        stamps.add( new AbilityStamp(this));
        particles.add(new ShockWave(PApplet.parseInt( owner.cx+cos(radians(owner.angle))*owner.w), PApplet.parseInt(owner.cy+sin(radians(owner.angle))*owner.w), 25, 20, 400, owner.playerColor));
        primeImage=new   AfterImage(PApplet.parseInt(owner.cx+cos(radians(splitAngle+90))*forceAmount*2), PApplet.parseInt( owner.cy+sin(radians(splitAngle+90))*forceAmount*2), cos(radians(splitAngle+90))*forceAmount*0.7f, sin(radians(splitAngle+90))*forceAmount*0.7f, splitAngle, 0, 0, 400, 3000, owner.playerColor, owner);
        primeImage.type=3;
        particles.add(primeImage);
        if (maxed) { 
          Buff b=  new Cold(owner, 4000, 0.2f);
          b.type=BuffType.ONCE;
          projectiles.add( new SniperBullet(owner, PApplet.parseInt( owner.cx+cos(radians(owner.angle))*100), PApplet.parseInt(owner.cy+sin(radians(owner.angle))*100), 50, owner.playerColor, 10, owner.angle, PApplet.parseInt(damage*0.75f)).addBuff(b) );
          timerActive=true;
          timer=0;
        } else {
          owner.MAX_ACCEL=owner.DEFAULT_MAX_ACCEL;
        ...
        owner.pushForce(forceAmount*0.7f, owner.keyAngle);
        owner.teleport( owner.keyAngle, forceAmount*2);
  ...
  public @Override
    ...
    if (timerActive) {
      timer+=2*timeBend;
      if (floor(timer)==2) {
        projectiles.add( new SniperBullet(owner, PApplet.parseInt( primeImage.x+cos(radians(owner.angle))*100), PApplet.parseInt(primeImage.y+sin(radians(owner.angle))*100), 10, owner.playerColor, 5, owner.angle, PApplet.parseInt(damage*0.1f)));
        projectiles.add( new SniperBullet(owner, PApplet.parseInt( owner.cx+cos(radians( owner.angle))*100), PApplet.parseInt(owner.cy+sin(radians( owner.angle))*100), 20, owner.playerColor, 5, owner.angle, PApplet.parseInt(damage*0.1f)));
      }
      if (floor(timer)==3) {
        projectiles.add( new SniperBullet(owner, PApplet.parseInt( primeImage.x+cos(radians(owner.angle))*100), PApplet.parseInt(primeImage.y+sin(radians(owner.angle))*100), 10, owner.playerColor, 5, owner.angle, PApplet.parseInt(damage*0.1f)));
        projectiles.add( new SniperBullet(owner, PApplet.parseInt( owner.cx+cos(radians( owner.angle))*100), PApplet.parseInt(owner.cy+sin(radians( owner.angle))*100), 20, owner.playerColor, 5, owner.angle, PApplet.parseInt(damage*0.1f)));
      ...
      if (timer>60) {
        projectiles.add( new SniperBullet(owner, PApplet.parseInt( primeImage.x+cos(radians(owner.angle))*100), PApplet.parseInt(primeImage.y+sin(radians(owner.angle))*100), 50, WHITE, 10, owner.angle, PApplet.parseInt(damage*0.75f)));
        projectiles.add( new SniperBullet(owner, PApplet.parseInt( owner.cx+cos(radians( owner.angle))*100), PApplet.parseInt(owner.cy+sin(radians( owner.angle))*100), 50, WHITE, 10, owner.angle, PApplet.parseInt(damage*0.75f)));
        timerActive=false;
        particles.add(new   AfterImage(PApplet.parseInt( primeImage.x), PApplet.parseInt( primeImage.y), 0, 0, owner.keyAngle, 0, 0, 400, 2000, owner.playerColor, owner) );
    ...
    if (!owner.stealth) {
      float temp=owner.angle;
      diffAngle=owner.keyAngle-owner.angle;
      splitAngle=owner.angle-diffAngle-90;
      noStroke();
      if (maxed) { 
        fill(WHITE);
        stroke(WHITE);
      ...
      pushMatrix();
      translate(PApplet.parseInt(owner.cx), PApplet.parseInt(owner.cy));
      rotate(radians(owner.keyAngle-90));
      //rect(-5, 0, 10, forceAmount*5);
      if (forceAmount>0) {
        strokeWeight(6);
        line( 0, forceAmount*5, -50, forceAmount*5-60);
        line( 0, forceAmount*5, +50, forceAmount*5-60);
      }
      popMatrix();
      pushMatrix();
...
class ChargeSlash extends Ability {//---------------------------------------------------    ChargeSlash   ---------------------------------
  //boolean charging;
  final float MAX_FORCE=85, damageFactor=0.3f;
  ...
  public @Override
    ...
    projectiles.add(new Slash(owner, PApplet.parseInt(owner.cx), PApplet.parseInt(owner.cy), 150, WHITE, PApplet.parseInt(3.3f*forceAmount), owner.angle, 50, 1.5f*forceAmount+50, cos(radians(owner.angle))*forceAmount*.5f, sin(radians(owner.angle))*forceAmount*.3f, PApplet.parseInt(forceAmount*damageFactor)+PApplet.parseInt(damageMod*.4f), true).hitOnce(true));
    projectiles.add(new Slash(owner, PApplet.parseInt(owner.cx), PApplet.parseInt(owner.cy), 150, WHITE, 280, owner.angle, -20, 70, cos(radians(owner.angle))*forceAmount, sin(radians(owner.angle))*forceAmount, PApplet.parseInt(forceAmount*damageFactor*2)+PApplet.parseInt(damageMod*4), false).hitOnce(true));
  ...
  public @Override
    void press() {
    if (cooldown<stampTime&&(!reverse || owner.reverseImmunity)&&(!freeze || owner.freezeImmunity) && energy>(0+activeCost)&& !hold && !active && !channeling && !owner.dead) {
      projectiles.add(new Slice(owner, PApplet.parseInt(owner.cx-cos(radians(owner.angle))*250), PApplet.parseInt(owner.cy-sin(radians(owner.angle))*250), 350, owner.playerColor, 180, owner.angle+145, 6, 130, cos(radians(owner.angle))*forceAmount*.5f, sin(radians(owner.angle))*forceAmount*.5f, PApplet.parseInt(20*damageFactor+damageMod*.1f), false));
                    play(chargeSound);
  ...
  public @Override
    ...
    if (!owner.stealth) {
      ...
      if (forceAmount>0) {
        strokeWeight(6);
        line( 0, forceAmount*11, -50, forceAmount*11-80);
        line( 0, forceAmount*11, +50, forceAmount*11-80);
      }
      popMatrix();
    }
...
class SkillPoint extends Ability {//---------------------------------------------------    SkillPoint   ---------------------------------
  //boolean charging;
  /*final float MAX_FORCE=85, damageFactor=0.2;
   ...
   boolean maxed;*/
  public int level, minLevel=1, maxLevel=1000;
  ...
  public void buy() {
    if (maxLevel>level && unlocked)level=0;
    updateSkillPoints();
  }
  public void downgrade() {
    level--;
...
class UpgradebleButton extends Button {
  ...
  public void update() {
    super.update();
    if (a.unlocked ) {
      if ( mouseX>x-size*.5f&&x+size*.5f>mouseX&&mouseY>y-size*.5f-plusSize*.2f*.5f&&y-size*.5f+plusSize*.2f*.5f>mouseY) { // plus
        //background(WHITE);
        plusHover=true;
        plusColor=GREEN;
        if (mousePressed && !pMousePressed) {
      ...
      if (mouseX>x-size*.5f&&x+size*.5f>mouseX&&mouseY>y+size*.5f-minusSize*.2f*.5f&&y+size*.5f+minusSize*.2f*.5f>mouseY) { // minus
        //background(WHITE);
        minusHover=true;
        minusColor=RED;
        if (mousePressed && !pMousePressed) {
  ...
  public void display() {
    ...
    if (a.unlocked) { 
      ...
      strokeWeight(4);
      fill(plusColor);
      rect(x, y-size*.5f, plusSize, plusSize*.25f);
      fill(minusColor);
      rect(x, y+size*.5f, minusSize, minusSize*.25f);
      fill(BLACK);
      text("+", x, y-size*.48f);
      text("-", x, y+size*.52f);
      textSize(24);
...
class ModeButton extends Button {
  ...
  public void hit() { // when button is pressed()
    gameMode=type;
    playerSetup();
    controllerSetup();
    resetGame();
...
class SettingButton extends Button {
  ...
  public void updateSettings() {
    ...
    if (order == abilitySettingsIndex[player.index]) {
      noFill();
      stroke(BLACK);
      rect(  x-size*0.5f, y-size*0.5f, size, size);
      indexHover=true;
...
class TempSlow extends Particle {
  ...
  public Particle setSoundTweek(boolean _b) {
    soundTweek=_b;
    speedControl.clear();
    if (soundTweek)speedControl.addSegment((reverse)?-1*timeBend:1*timeBend, 800); //now slow
    return this;
...
class AfterImage extends Particle {
  ...
  float  DEFAULT_FRICTION_FACTOR=0.1f,FRICTION_FACTOR=DEFAULT_FRICTION_FACTOR;
  Player owner;
  ...
  public @Override
    ...
    if (!owner.stealth && !owner.stationary) {
      ...
      if (dist(owner.cx, owner.cy, x, y)>60) {
        ...
        Shield s=new Shield( owner, PApplet.parseInt( owner.cx+cos(radians(ang))*60), PApplet.parseInt(owner.cy+sin(radians(ang))*60), owner.playerColor, 1400, ang, 0);
        s.size=45;
        projectiles.add( s);
...
class Nova extends Ability {//---------------------------------------------------    bullet   ---------------------------------
  ...
  public @Override
    ...
    if (cooldown>50 && !owner.dead) {
      ...
      projectiles.add( new Slash(owner, PApplet.parseInt( owner.cx+sin(owner.keyAngle)*60), PApplet.parseInt(owner.cy+cos(owner.keyAngle)*60), 40, owner.playerColor, 200, owner.angle-100, -24, 150, sin(owner.keyAngle)*10, cos(owner.keyAngle)*10, 4, true));
      projectiles.add( new Slash(owner, PApplet.parseInt( owner.cx+sin(owner.keyAngle)*60), PApplet.parseInt(owner.cy+cos(owner.keyAngle)*60), 40, owner.playerColor, 200, owner.angle-280, -24, 150, sin(owner.keyAngle)*10, cos(owner.keyAngle)*10, 4, true));
    }
  }
  public @Override
    void passive() {
  ...
  public @Override
    ...
    if (cooldown>interval) {
      ...
      if (trigger) {
        ...
        owner.slowImmunity=true;
        particles.add(new TempSlow(1200, 0.025f, 1.05f));
        trigger=false;
        cooldown=0;
      }
    }
...
class FlashBang extends Projectile implements Reflectable, Containable, Container {//----------------------------------------- Bomb objects ----------------------------------------------------

  float  friction=0.98f, shakeness;
  int blastForce=30, flick;
  boolean friendlyFire;
  Projectile parent;
  Containable[] payload;
  FlashBang(Player _owner, int _x, int _y, int _size, int _projectileColor, int  _time, float _angle, float _vx, float _vy, int _damage, boolean _friendlyFire) {
    super(_owner, _x, _y, _size, _projectileColor, _time);
    angle=_angle;
    damage=_damage;
    blastRadius=800;
  ...
  public @Override
    ...
    if ( !dead) {         
      ...
      if (payload!=null) {
        ...
        //   particles.add(new ShockWave(int(x), int(y), int(blastRadius*0.4), 32, 150, (friendlyFire)? BLACK:color(owner.playerColor)));
        particles.add(new ShockWave(PApplet.parseInt(x), PApplet.parseInt(y), PApplet.parseInt(blastRadius*1), 16, 200, WHITE));
        particles.add(new ShockWave(PApplet.parseInt(x), PApplet.parseInt(y), PApplet.parseInt(blastRadius*1), 36, 200, WHITE));

        particles.add(new ShockWave(PApplet.parseInt(x), PApplet.parseInt(y), PApplet.parseInt(blastRadius*0.8f), 32, 150, (friendlyFire)? BLACK:color(owner.playerColor)));
        particles.add(new Flash(3000, 1, WHITE));
        particles.add(new TempSlow(200, 0.5f, 1.05f));
...
class Rocket extends Bomb implements Reflectable, Destroyable, Container {//----------------------------------------- Bomb objects ----------------------------------------------------
  ...
  public @Override
    ...
    super.hit(enemy);
    particles.add( new TempSlow(10, 0.15f, 1.05f).setSoundTweek(false));
    super.fizzle();
    // fizzle();
    deathTime=stampTime;   // projectile is dead on collision
    dead=true;
...
class Missile extends Rocket implements Reflectable {//----------------------------------------- Missile objects ----------------------------------------------------
  ...
  Player target;
  Missile(Player _owner, int _x, int _y, int _size, int _projectileColor, int  _time, float _angle, float _vx, float _vy, int _damage, boolean _friendlyFire) {
    super(_owner, _x, _y, _size, _projectileColor, _time, _angle, _vx, _vy, _damage, _friendlyFire);
    for (int i=0; i<2; i++) {
      particles.add(new Particle(PApplet.parseInt(x), PApplet.parseInt(y), random(10)-5+vx*0.5f, random(10)-5+vy*0.5f, PApplet.parseInt(random(20)+5), 800, 255));
    }
...
class Slash extends Projectile implements Destroyer {//----------------------------------------- Slash objects ----------------------------------------------------
  ...
  public Projectile hitOnce(boolean _hitOnce) {
    hitOnce=_hitOnce;
    return this;
  }
...
class Boomerang extends Projectile implements Reflectable, Destroyer {//----------------------------------------- Boomerang objects ----------------------------------------------------
  ...
  public void destroying(Projectile destroyerP) {
    for (int i=0; i<2; i++) {
...
public void spawningSetup() {
  ...
  spawnList.add(new Spawner(new Object[]{new FollowDrone(players.size(), halfWidth, halfHeight, PApplet.parseInt(playerSize*1), PApplet.parseInt(playerSize*1.2f), 20, 440, 1, new MissileLauncher(), new MpRegen(), new  Reward(10, false))}
...
public void bossRushSetup() {
  ...
  bossList.add(new FollowDrone(players.size(), halfWidth, halfHeight, PApplet.parseInt(playerSize*1), PApplet.parseInt(playerSize*1), 15, PApplet.parseInt(1000*DIFFICULTY_LEVEL), 4, new HpRegen(10, 10), new RapidFire(), new RapidFire(), new MachineGun(), new MachineGun(), new  Reward(25, false))); //, new MissileLauncher()

  bossList.add(new FollowDrone(players.size(), halfWidth, halfHeight, PApplet.parseInt(playerSize*1), PApplet.parseInt(playerSize*1), 15, PApplet.parseInt(1200*DIFFICULTY_LEVEL), 1, new Sluggun(), new MpRegen(), new Sluggun(), new PanicBlink(), new  Reward(25, false))); //, new MissileLauncher()
  bossList.add(new FollowDrone(players.size(), halfWidth, halfHeight, PApplet.parseInt(playerSize*1.75f), PApplet.parseInt(playerSize*1.75f), 15, PApplet.parseInt(1100*DIFFICULTY_LEVEL), 1, new Bazooka(), new Bazooka(), new Bazooka(), new  Reward(25, false))); //, new MissileLauncher()
  ...
  bossList.add(new FollowDrone(players.size(), halfWidth, halfHeight, PApplet.parseInt(playerSize*1.75f), PApplet.parseInt(playerSize*1.75f), 15, PApplet.parseInt(600*DIFFICULTY_LEVEL), 1, new MarbleLauncher(), new Torpedo(), new  Reward(25, false))); //, new MissileLauncher()
...
public void initSound(SamplePlayer sp) {
 // g.addInput(sp);
...
class QuadTree {
  ...
  public void subdivide() {
    int hw=PApplet.parseInt(w*0.5f), hh=PApplet.parseInt(h*0.5f);
...
public void previousModeButton() {
...
public void loadProgress() throws Exception {
  ...
  skillpointsButton=new UpgradebleButton(new SkillPoint(), PApplet.parseInt(shopXEdgePadding+(i*shopXInterval)%(width-shopXEdgePadding*2)), PApplet.parseInt(shopYEdgePadding+PApplet.parseInt(i*shopXInterval/(width-shopXEdgePadding*2))*shopYInterval), 70);
...
class Player implements Cloneable {
  ...
  public Player setClone(boolean b) {
    clone=b;
...
public void settingsInput(){
 if (gameMode==GameType.SETTINGS) {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "multiplayer_ball_game" };
    if (passedArgs != null) {
